//! RISC-V RV64 interpreter -- executes YOYO-emitted RISC-V machine code
//! and extracts the final state for DDC comparison against the TIR simulator.
//!
//! The emitted code uses:
//!   - state base register x5 (t0, set by the ELF startup stub)
//!   - state slots as 64-bit words at [x5 + slot*8]
//!   - scratch registers x6/x7/x10/x11
//!   - CMP stores a in x10, b in x11; JCC uses x10/x11
...
impl Cpu {
    ...
    fn step(&mut self) -> Option<ExecExitReason> {
        ...
        if opcode == 0x17 {
            ...
            self.set_reg(rd, (self.pc as i64 + imm) as u64);
            self.pc += 4;
            return None;
        }
        ...
        if opcode == 0x6F {
            ...
            let target = (self.pc as i64 + imm as i64) as u64;
        ...
        // JALR (0x67)
        if opcode == 0x67 {
            let imm12 = ((insn >> 20) as i32) << 20 >> 20; // sign-extend 12-bit
            let temp = self.pc + 4;
        ...
        if opcode == 0x63 {
            ...
            let cond = match funct3 {
                ...
                4 => (self.r(rs1) as i64) < (self.r(rs2) as i64), // BLT
                5 => (self.r(rs1) as i64) >= (self.r(rs2) as i64), // BGE
        ...
        if opcode == 0x03 {
            ...
            let addr = (self.r(rs1) as i64 + imm12 as i64) as u64;
        ...
        if opcode == 0x13 {
            ...
            let result = match funct3 {
                0 => (rs1v as i64 + imm12 as i64) as u64,    // ADDI
                1 => rs1v << ((insn >> 20) & 0x3F),          // SLLI (shamt encoded in imm)
                2 => {
                    if (rs1v as i64) < (imm12 as i64) { 1 } else { 0 } // SLTI
                }
                3 => { if rs1v < (imm12 as u64) { 1 } else { 0 } }   // SLTIU
                4 => rs1v ^ (imm12 as u32 as u64),            // XORI
                5 => {
                    let shamt = (insn >> 20) & 0x3F;
                    let shift_type = (insn >> 30) & 0x1;
                    if shift_type == 0 { rs1v >> shamt }      // SRLI
                    else { ((rs1v as i64) >> shamt) as u64 }  // SRAI
                }
                6 => rs1v | (imm12 as u32 as u64),            // ORI
                7 => rs1v & (imm12 as u32 as u64),            // ANDI
                _ => 0,
            };
        ...
        if opcode == 0x33 {
            ...
            let result = match (funct3, funct7) {
                ...
                (1, 0x00) => rs1v << (rs2v & 0x3F),             // SLL
                (2, 0x00) => if (rs1v as i64) < (rs2v as i64) { 1 } else { 0 }, // SLT
                (3, 0x00) => if rs1v < rs2v { 1 } else { 0 },   // SLTU
                (4, 0x00) => rs1v ^ rs2v,                       // XOR
                (5, 0x00) => rs1v >> (rs2v & 0x3F),             // SRL
                (5, 0x20) => ((rs1v as i64) >> (rs2v & 0x3F)) as u64, // SRA
                (6, 0x00) => rs1v | rs2v,                       // OR
                (7, 0x00) => rs1v & rs2v,                       // AND
                (0, 0x01) => rs1v.wrapping_mul(rs2v),           // MUL
                (1, 0x01) => {
                    // MULH: (signed)signed
                    ((rs1v as i64 as i128).wrapping_mul(rs2v as i64 as i128) >> 64) as u64
                }
                (2, 0x01) => {
                    // MULHSU: signed x unsigned
                    let a = rs1v as i64 as i128;
                    let b = rs2v as i128;
                    (a.wrapping_mul(b) >> 64) as u64
                }
                (3, 0x01) => {
                    // MULHU: unsigned
                    ((rs1v as u128).wrapping_mul(rs2v as u128) >> 64) as u64
                }
                (4, 0x01) => if rs2v == 0 { 0 } else { rs1v.wrapping_div(rs2v) }, // DIV
                (5, 0x01) => if rs2v == 0 { 0 } else { rs1v.wrapping_div(rs2v) }, // DIVU
                (6, 0x01) => if rs2v == 0 { rs1v } else { rs1v.wrapping_rem(rs2v) }, // REM
                (7, 0x01) => if rs2v == 0 { rs1v } else { rs1v.wrapping_rem(rs2v) }, // REMU
        ...
        // FENCE / FENCE.I (opcode 0x0F) 鈥?treat as NOP
        if opcode == 0x0F {
            self.pc += 4;
            return None;
        }
...
pub fn run_riscv_elf(elf_bytes: &[u8]) -> ExecResult {
    ...
    // Skip the 16-byte startup stub and set x5 = data_va
    let mut cpu = Cpu::new(mem, e_entry + 16);
    cpu.regs[5] = data_va;
    let exit_reason = cpu.run();

    // Read state from memory at x5 (data_va)
