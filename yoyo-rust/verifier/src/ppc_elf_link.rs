...
pub struct ElfPpcImage {
    pub bytes: Vec<u8>,
}

/// Wrap raw code (+ optional data) in a PPC64LE ELF64 executable.
pub fn link_ppc_elf(code: &[u8], data: &[u8]) -> IsaResult<ElfPpcImage> {
    ...
    // Virtual layout:
    //   .text @ 0x10000000, .data @ 0x1001000
    let text_va = 0x10000000u64;
    let data_va = 0x1001000u64;
    ...
    write_u16(&mut img, 18, 0x142);           // EM_PPC64
...
mod tests {
    ...
    #[test]
    fn ppc_elf_has_magic() {
        let elf = link_ppc_elf(&[0x60, 0x00, 0x00, 0x00], &[]).unwrap();
        ...
        assert_eq!(e_machine, 0x142); // EM_PPC64
    }

    #[test]
    fn ppc_elf_entry_and_phnum() {
        let elf = link_ppc_elf(&[0x60, 0x00, 0x00, 0x00], &[]).unwrap();
        let e_entry = u64::from_le_bytes(elf.bytes[24..32].try_into().unwrap());
        assert_eq!(e_entry, 0x10000000);
