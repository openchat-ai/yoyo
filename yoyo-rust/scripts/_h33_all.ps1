...
# Link run, capture verbose
$out = & cargo run -q -p verifier --bin yoyo -- link --target=stub F:/yoyo/yoyo/tests/golden/_scratch_h33.ty $bin 2>&1
$ec = $LASTEXITCODE
Write-Host "EXIT=$ec"
$out | Where-Object { $_ -is [string] -and $_.Trim() -ne '' -and $_ -notmatch '^warning|^-->' } | ForEach-Object { Write-Host "C: $_" }
# Now read the file
