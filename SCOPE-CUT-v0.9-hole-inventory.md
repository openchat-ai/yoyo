...
**Post-v1.0 OW-H00 re-eval（PR #8）：** JS hex template lockstep；**three_peer_full=DIFF**（body EQUAL）。**OW-H00 CUT** — 撤销 PR #6 CLOSED（stub bytes JS≠Rust）。
...
|----|------|-------------|---------------------------|----------------|
| **OW-H00** | H_00 entry slot（18 B）+ manual-map stub tail | **CUT** | `three_peer_full_text=EQUAL` · full `.text` JS=Rust **18944** B | slot **JMP+NOP aligned**；full `.text` **DIFF**（JS `a9b4cdc8` ≠ Rust `72c27c9f`） |
...
当前基线下 **全部为 CUT**（7）。PR #6 OW-H00 CLOSED **已撤销**（PR #8 manual-map 致 three_peer_full DIFF）。
...
- ❌ 把 SCOPE-CUT 写成失败或倒退
- ❌ 维持 PR #6「OW-H00 CLOSED」话术（PR #8 后 three_peer_full **DIFF**）
...
## 观测基线（2026-08-29 · post PR #8 merge · master `1598cad`）

| Monitor | Value |
|---------|-------|
| selfhost-body compared | **17805** B EQUAL |
| full `.text` JS↔Rust | **DIFF** · compared **18944** B |
...
| Disposition | **OW-\* all CUT**（closed=0 cut=7） |
| Gates | body-ddc · gen12 · stage17-wireup · stage10-linux GREEN |

---

*Stage 15-A · post PR #8：OW-H00 撤销 CLOSED（three_peer_full DIFF）· OW-IAT manual-map wired（仍 CUT）· OW-STUB stub_nz=905*
