# SCOPE-CUT — v0.9 hole inventory（Stage 15-A 收口）
...
## 观测基线（2026-08-29 · Stage 15-A · gate GREEN）
...
| Lock pin | `0275802d…` Decision #25（本收口不改 `yoyo.ty`） |
| Disposition | **OW-\* + REL-\* all CUT**（closed=0 cut=7） |
| Gate | `stage15-hole-inventory.ps1 -SkipBuild` exit **0** · `HOLE_INVENTORY status=ACTIVE` |
| No-regress | nested stage14-A exit 0 · stage14-B lock harden PINNED |

---

*Stage 15-A · 打破后门魔咒：洞从「清单」变成「CLOSED|CUT + 可脚本钉」*
