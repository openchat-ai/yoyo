# YOYO v1.0 — Release Boundary (Owner One-Pager)
...
>
> **Sources:** `SCOPE-v1.0.md`, `STAGE16_OWNER_CHECKLIST.md`, `SCOPE-CUT-v1.0-hole-inventory.md`, `DETECTION-BANLIST-v1.0.md`, `RELEASE-v0.9.md` · baseline 2026-08-29.
...
**Honest boundary:** DDC detects **output divergence** across independent peers under equal inputs. That is a practical **detection** bar — **not** Thompson immunity or proof of purity. **HOLE_INVENTORY_V10 status=FINAL** · **closed=0 cut=7**. **Still Rust-compiled runtime + host LoadLibraryA / libdl** (not YOYO-built). **Seed is still Rust-emitted** (`yoyo.exe` host). Full `.text` peer compare **DIFF** (expected). Stub OS remain stub (not production I/O). Comparable EQUAL remains **selfhost-body window only** (17805 B).

**Banlist cite:** `DETECTION-BANLIST-v1.0.md`（ACTIVE）.  
**Inventory cite:** `SCOPE-CUT-v1.0-hole-inventory.md`（FINAL · closed=0 cut=7）.

---

## What IS in v1.0

**Product identity (honest):** An **auditable x86-64 compiler ISA** that keeps the v0.9 inventory / pre-run / regression baseline, then **FINAL SCOPE-CUT** + **detection-only RELEASE wording** — remaining host holes stay **CUT**, not pretended CLOSED.

### Core deliverables (increment over v0.9)

| Area | v1.0 includes |
|------|----------------|
| **SCOPE-CUT FINAL (Stage 16-A)** | `SCOPE-CUT-v1.0-hole-inventory.md` + `stage16-scope-cut-finalize.ps1` (+ `stage16-a`); OW-\*/REL-\* each `FINAL_HOLE … disposition=CLOSED\|CUT`; **closed=0 cut=7** · `HOLE_INVENTORY_V10 status=FINAL` |
...
| **v0.9 regression retained (Stage 16-C)** | `stage16-v09-regress.ps1` (+ `stage16-c`); serial stage15 A/B/C + stage14–9 + all/lock/gen12/fullbody + Stage 16 A/B + WSL; named `-SkipBuild`; **zero parallel cargo** |
| **v0.9 baseline retained** | hole inventory · pre-run · v0.8 regress · Lock Decision #25 · selfhost-body · LoadLibrary/libdl · gen12 · fullbody · Win/Linux pure M4 |

### Remaining CUT list (must ship in RELEASE — from Stage 16-A FINAL)

| ID | Disposition | Notes |
|----|-------------|-------|
| **OW-H00** | **CUT** | H_00 entry slot; full `.text` DIFF; body skips slot |
| **OW-STUB** | **CUT** | Rust extract stub_tail_nonzero (obs. 159) |
| **OW-RT** | **CUT** | Embedded Rust `yoyo_runtime.dll` (154624; exact embed) |
| **OW-IAT** | **CUT** | LoadLibraryA / libdl host trampoline still present |
| **OW-SEED** | **CUT** | Seed still Rust-emitted (`yoyo.exe`) |
| **REL-FULLTEXT** | **CUT** | full `.text` not a graduation EQUAL claim |
| **REL-STUBOS** | **CUT** | Plan9/FreeBSD/Haiku/Serenity stub — not production I/O |

### Trust-chain anchors (documented SHA)

| Monitor | Value | Notes |
|---------|-------|-------|
| **Lock pin (`yoyo.ty`)** | `0275802d2b4459e6ece0801a73af3e988d203c6a34dacfa382f2e48fe8ad43cb` | Decision #25 — **unchanged** (Stage 16 did not edit locked source) |
...
| **Hole inventory** | **FINAL** · closed=0 · cut=7 | All seven **CUT** (honest; no fake CLOSED) |
| **Detection banlist** | **ACTIVE** | Gate: `stage16-detection-wording.ps1` |
| **Linux trampoline** | size **9768** (v0.5+) | Still host libdl path |

### Lock / Relock (v1.0 graduation) — Decision

Stage 16 **A/B/C/D did not modify `yoyo/projects/yoyo.ty`**. Trust gains came from **FINAL SCOPE-CUT**, **detection banlist + RELEASE wording**, and **v0.9 regression harness** — not a source-body edit. Therefore:

- **No Relock required** — Decision #25 pin remains authoritative
- **Verified 2026-08-29:** `verify-lock-pin.ps1` exit 0 · `stage14-lock-harden.ps1 -SkipBuild` · `LOCK_HARDEN status=PINNED decision=25 relock=NO ty_eq_lock=YES` · `stage16-v09-regress.ps1 -SkipBuild` ALL_GREEN (stamp **02:14:21**) · Stage 16-D accept
...
| Door | Trust gain |
|------|------------|
| **A** | v0.9 ACTIVE inventory becomes v1.0 FINAL SCOPE-CUT — each OW-\*/REL-\* machine `CLOSED\|CUT` (here all CUT) |
| **B** | Outward RELEASE without claiming Thompson-proof / fully closed / fake EQUAL — banlist machine-checked |
| **C** | v0.9 gates remain green — FINAL SCOPE-CUT / banlist expansion does not buy regressions |
...
**Stage 16-D re-verify (2026-08-29):** Lock PINNED · `stage16-v09-regress.ps1 -SkipBuild` ALL_GREEN (serial; stamp 02:14:21; no parallel cargo).

### Docs that belong in v1.0

- `SCOPE-v1.0.md` — v1.0 boundary one-pager (graduated)
- `SCOPE-CUT-v1.0-hole-inventory.md` — FINAL CLOSED|CUT inventory (closed=0 cut=7)
- `DETECTION-BANLIST-v1.0.md` — outward forbidden claims (ACTIVE)
- `STAGE16_OWNER_CHECKLIST.md` — Stage 16 A→D graduation board
- `RELEASE-v1.0.md` — this file
- `RELEASE-NOTES-v1.0.md` — short external notes
- Upstream retained: `RELEASE-v0.9.md` · `SCOPE-CUT-v0.9-hole-inventory.md`
...
| **Thompson-proof / DDC ⇒ provably correct** | Forbidden claim — see banlist |
...
| **Stage 17 feature track** | ROADMAP endpoint — no invent Stage 17 |
| **TheoryManifest / CDS theater** | N.4.1 FORBIDDEN |

### Remaining surface (honest — HOLE_INVENTORY_V10 FINAL; not pretended CLOSED)
...
### Misleading claims — forbidden in v1.0 release notes

Do **not** publish wording that implies:

- “Thompson-proof” or “immune to compiler backdoors”
- Holes are **fully closed** / 洞已全关 / closed=7 (they remain **CUT**, closed=0)
...
**Machine banlist:** `DETECTION-BANLIST-v1.0.md` (`BAN id=THOMPSON|FULLY_CLOSED|FAKE_EQUAL|…`).
...
Stage 16-D re-verify: `verify-lock-pin.ps1` · `stage14-lock-harden.ps1 -SkipBuild` PINNED · `stage16-v09-regress.ps1 -SkipBuild` ALL_GREEN 2026-08-29 02:14:21. Prior A/B/C gates ALL_GREEN.
...
- [x] A — SCOPE-CUT FINAL (closed=0 cut=7)
- [x] B — detection wording / banlist
- [x] C — v0.9 regression retained
- [x] D — v1.0 graduation gate + docs + tag

### 4. Release notes honesty pass

- [x] No ROADMAP items listed as “done in v1.0”
