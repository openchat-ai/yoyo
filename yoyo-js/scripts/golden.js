...
const {
  ...
  setEmitPlatform,
  getEmitPlatform,
} = require('../src/platform/encode-x64');
const {
  isMovabsStoreStub,
  emitWin32Alloc,
  emitLinuxAlloc,
} = require('../src/platform/platform-io');
...
/**
 * Stage 9-B: stub path remains movabs+store (G-SM-IO / cross-peer golden).
 * Fail-closed if default encode platform is polluted to win32 during golden.
 */
function checkPEERIOSTUB() {
  const prev = getEmitPlatform();
  try {
    setEmitPlatform('stub');
    const got = Buffer.from(encodeOp(0x20, [0x50, 0x1000], false));
    if (!isMovabsStoreStub(got) || got.length !== 17) {
      return { id: 'PEER-IO-STUB', ok: false, detail: `stub ALLOC must be 17B movabs+store, got ${got.length}B ${hexOf(got)}` };
    }
    const pin = loadExpectedHex(path.join(GOLDEN_DIR, 'expected', 'selfhost_min_io.code.hex'));
    // pin is 4×18B handlers; ALLOC body is bytes [18..35) without trailing RET of first handler…
    // Compare ALLOC opcode body only: movabs 0x1000 + store S[0x50]
    const want = pin.slice(18, 35);
    if (!got.equals(want)) {
      return { id: 'PEER-IO-STUB', ok: false, detail: `stub ALLOC mismatch got ${hexOf(got)} want ${hexOf(want)}` };
    }
    return { id: 'PEER-IO-STUB', ok: true, detail: 'stub ALLOC=17B movabs+store (G-SM-IO contract)' };
  } finally {
    setEmitPlatform(prev);
  }
}

/**
 * Stage 9-B: win32 production emit must match Rust platform_io (IAT call, not stub).
 */
function checkPEERIOWIN32() {
  const prev = getEmitPlatform();
  try {
    setEmitPlatform('win32');
    const alloc = Buffer.from(encodeOp(0x20, [0x50, 0x1000], false));
    const load = Buffer.from(encodeOp(0x50, [0x50, 0], false));
    const write = Buffer.from(encodeOp(0x51, [0x50, 0, 0x51], false));
    if (isMovabsStoreStub(alloc)) {
      return { id: 'PEER-IO-WIN32', ok: false, detail: 'win32 ALLOC still movabs+store stub (DDC blind zone)' };
    }
    const ref = Buffer.from(emitWin32Alloc(0x50, 0x1000));
    if (!alloc.equals(ref)) {
      return { id: 'PEER-IO-WIN32', ok: false, detail: 'ALLOC encodeOp≠emitWin32Alloc' };
    }
    // Signature: call qword [r15+disp] = 41 FF 97
    const hasIat = (b) => {
      for (let i = 0; i + 2 < b.length; i++) {
        if (b[i] === 0x41 && b[i + 1] === 0xff && b[i + 2] === 0x97) return true;
      }
      return false;
    };
    if (!hasIat(alloc) || !hasIat(load) || !hasIat(write)) {
      return { id: 'PEER-IO-WIN32', ok: false, detail: 'missing call [r15+iat] in ALLOC/LOAD/WRITE' };
    }
    if (alloc.length !== 41 || load.length !== 144 || write.length !== 127) {
      return {
        id: 'PEER-IO-WIN32',
        ok: false,
        detail: `len mismatch alloc=${alloc.length} load=${load.length} write=${write.length} (want 41/144/127)`,
      };
    }
    return {
      id: 'PEER-IO-WIN32',
      ok: true,
      detail: `ALLOC=${alloc.length}B LOAD=${load.length}B WRITE=${write.length}B IAT (Rust-aligned)`,
    };
  } finally {
    setEmitPlatform(prev);
  }
...
/**
 * Stage 9-B: linux emit uses syscall (0F 05), not movabs+store stub.
 */
function checkPEERIOLINUX() {
  const prev = getEmitPlatform();
  try {
    setEmitPlatform('linux');
    const alloc = Buffer.from(encodeOp(0x20, [0x50, 0x1000], false));
    if (isMovabsStoreStub(alloc)) {
      return { id: 'PEER-IO-LINUX', ok: false, detail: 'linux ALLOC still movabs+store stub' };
    }
    const ref = Buffer.from(emitLinuxAlloc(0x50, 0x1000));
    if (!alloc.equals(ref)) {
      return { id: 'PEER-IO-LINUX', ok: false, detail: 'ALLOC encodeOp≠emitLinuxAlloc' };
    }
    let syscall = false;
    for (let i = 0; i + 1 < alloc.length; i++) {
      if (alloc[i] === 0x0f && alloc[i + 1] === 0x05) { syscall = true; break; }
    }
    if (!syscall) {
      return { id: 'PEER-IO-LINUX', ok: false, detail: 'linux ALLOC missing syscall 0F 05' };
    }
    return { id: 'PEER-IO-LINUX', ok: true, detail: `ALLOC=${alloc.length}B syscall (Rust-aligned)` };
  } finally {
    setEmitPlatform(prev);
  }
}
