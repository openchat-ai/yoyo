# stage15-v08-regress.ps1 — Stage 15-C: v0.8 regression must not regress
# Fail-closed serial gate (no parallel cargo; & not Start-Process|Out-Null).
# Covers: stage14 A/B/C + stage13–9 + yoyo.exe test all/lock/gen12/fullbody
#   + Stage 15 A (hole inventory); Stage 15 B substance = A + stage14-v07 umbrella
#   (re-listed here as first-class gates — do not nest stage15-prerun to avoid
#   recursive re-entry into this same coverage).
...
$WorkDir = Join-Path $Root "scripts\_stage15-v08-regress"
...
function Write-Summary([string]$Status) {
    ...
    $lines.Add("Stage 15-C v0.8 regression $Stamp") | Out-Null
    $lines.Add("") | Out-Null
    ...
    $lines.Add("") | Out-Null
    $lines.Add("Trust chain: Stage 15 A/B expansion must not drop v0.8 baseline.") | Out-Null
    $lines.Add("  stage14 A/B + stage13/12/11/10/9 + fullbody/lock/gen12 + Lock pin") | Out-Null
    $lines.Add("  + Stage 15-A hole inventory stay green under one serial gate") | Out-Null
    $lines.Add("  (one cargo max, then yoyo.exe / -SkipBuild).") | Out-Null
    $lines.Add("Honest unchanged: HOLE_INVENTORY CUT remain; Rust runtime + LoadLibrary;") | Out-Null
    $lines.Add("  full .text may DIFF; SCOPE-CUT ACTIVE; Lock pin Decision #25;") | Out-Null
    $lines.Add("  DDC = detection not proof; seed still Rust-emitted.") | Out-Null
...
function Fail-Out {
    ...
    Write-Host "Stage 15-C: RED" -ForegroundColor Red
...
function Wait-ZeroCargo {
    ...
    while ($true) {
        ...
        if ((Get-Date) -ge $deadline) {
            Write-Host "Stage 15-C: RED (timeout waiting for cargo/rustc)" -ForegroundColor Red
...
Write-Host "=== Stage 15-C: v0.8 regression gate ==="
Write-Host "  serial: wait cargo → one release build → yoyo.exe test* → gates -SkipBuild"
Write-Host "  stamp: $Stamp"

# Fail-closed if another stage15-v08-regress still owns shared _stage* workdirs.
...
if (Test-Path $LockPath) {
    ...
    if ($lockAgeMin -lt 120) {
        ...
        Write-Host ("Stage 15-C: RED (concurrent driver.lock age={0:N1}m — {1})" -f $lockAgeMin, $lockBody.Trim()) -ForegroundColor Red
...
if ($SkipWsl) {
    Write-Host "Stage 15-C: RED (SkipWsl forbidden for graduation — platform-blind)" -ForegroundColor Red
...
if (-not $SkipBuild) {
    ...
    if (-not (Test-Path $Yoyo)) {
        Write-Host "Stage 15-C: RED (yoyo.exe missing and -SkipBuild)" -ForegroundColor Red
        Fail-Out
    }
...
if (-not (Test-Path $Yoyo)) {
    Write-Host "Stage 15-C: RED (yoyo.exe missing after build)" -ForegroundColor Red
    Fail-Out
}
...
# Order mirrors stage14-v07-regress (Stage 14-C), then Stage 15-A.
# Stage 15-B (prerun) is not nested — it would re-enter stage14-v07 / hole-inventory;
# C already lists that coverage first-class (B-as-appropriate = same gates, one driver).
$scriptGates = @(
    "verify-lock-pin.ps1",
...
try {
    foreach ($name in $scriptGates) {
        ...
        if (-not (Test-Path $gatePath)) {
            Write-Host ("Stage 15-C: RED (missing gate {0})" -f $name) -ForegroundColor Red
    ...
    Write-Host "Stage 15-C: GREEN" -ForegroundColor Green
    exit 0
} finally {
    Remove-Item -LiteralPath $LockPath -Force -ErrorAction SilentlyContinue
}
