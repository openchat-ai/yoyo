# stage16-v09-regress.ps1 — Stage 16-C: v0.9 regression must not regress
# Fail-closed serial gate (no parallel cargo; & not Start-Process|Out-Null).
# Covers: stage15 A/B/C + stage14–9 + yoyo.exe test all/lock/gen12/fullbody
#   + WSL + Stage 16 A (SCOPE-CUT FINAL) + Stage 16 B (detection wording).
# Stage 15-B (prerun) / Stage 15-C (v08-regress) are not nested — they would
#   re-enter this same coverage; C lists that coverage first-class.
...
$WorkDir = Join-Path $Root "scripts\_stage16-v09-regress"
...
function Write-Summary([string]$Status) {
    ...
    $lines.Add("Stage 16-C v0.9 regression $Stamp") | Out-Null
    $lines.Add("") | Out-Null
    ...
    $lines.Add("") | Out-Null
    $lines.Add("Trust chain: Stage 16 A/B expansion must not drop v0.9 baseline.") | Out-Null
    $lines.Add("  stage15 A/B/C + stage14/13/12/11/10/9 + fullbody/lock/gen12 + Lock pin") | Out-Null
    $lines.Add("  + Stage 16-A FINAL SCOPE-CUT + Stage 16-B detection wording") | Out-Null
    $lines.Add("  stay green under one serial gate (one cargo max, then yoyo.exe / -SkipBuild).") | Out-Null
    $lines.Add("Honest unchanged: HOLE_INVENTORY_V10 FINAL closed=0 cut=7; Rust runtime +") | Out-Null
    $lines.Add("  LoadLibrary; full .text may DIFF; Lock pin Decision #25;") | Out-Null
    $lines.Add("  DDC = detection not proof; seed still Rust-emitted; RELEASE still DRAFT.") | Out-Null
...
function Fail-Out {
    ...
    Write-Host "Stage 16-C: RED" -ForegroundColor Red
...
function Wait-ZeroCargo {
    ...
    while ($true) {
        ...
        if ((Get-Date) -ge $deadline) {
            Write-Host "Stage 16-C: RED (timeout waiting for cargo/rustc)" -ForegroundColor Red
...
Write-Host "=== Stage 16-C: v0.9 regression gate ==="
Write-Host "  serial: wait cargo → one release build → yoyo.exe test* → gates -SkipBuild"
Write-Host "  stamp: $Stamp"

# Fail-closed if another stage16-v09-regress still owns shared _stage* workdirs.
...
if (Test-Path $LockPath) {
    ...
    if ($lockAgeMin -lt 120) {
        ...
        Write-Host ("Stage 16-C: RED (concurrent driver.lock age={0:N1}m — {1})" -f $lockAgeMin, $lockBody.Trim()) -ForegroundColor Red
...
if ($SkipWsl) {
    Write-Host "Stage 16-C: RED (SkipWsl forbidden for graduation — platform-blind)" -ForegroundColor Red
...
if (-not $SkipBuild) {
    ...
    if (-not (Test-Path $Yoyo)) {
        Write-Host "Stage 16-C: RED (yoyo.exe missing and -SkipBuild)" -ForegroundColor Red
        Fail-Out
    }
...
if (-not (Test-Path $Yoyo)) {
    Write-Host "Stage 16-C: RED (yoyo.exe missing after build)" -ForegroundColor Red
    Fail-Out
}
...
# Order mirrors stage15-v08-regress (Stage 15-C), then Stage 16 A/B.
# Stage 15-B (prerun) / Stage 15-C (v08) are not nested — they would re-enter
# the same stage14–9 + hole-inventory coverage; C lists that first-class.
$scriptGates = @(
    "verify-lock-pin.ps1",
    ...
    "stage16-scope-cut-finalize.ps1",
    "stage16-detection-wording.ps1"
)
...
try {
    foreach ($name in $scriptGates) {
        ...
        if (-not (Test-Path $gatePath)) {
            Write-Host ("Stage 16-C: RED (missing gate {0})" -f $name) -ForegroundColor Red
    ...
    Write-Host "Stage 16-C: GREEN" -ForegroundColor Green
    exit 0
} finally {
    Remove-Item -LiteralPath $LockPath -Force -ErrorAction SilentlyContinue
}
