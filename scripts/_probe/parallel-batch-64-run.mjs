/**
 * parallel-batch-64 probe — scratch-only fresh picks after H_477..H_484.
 * Mirror parallel-batch-63-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_484. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Start 158 ADD triad + start 158 SUB triad + start 160 LDB (dd=50/51).
const picks = [
  {
    name: 'addimm_h50_158',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x158',
    body: '62 50 158',
    expected: pinOf(0x62, [0x50, 0x158]),
  },
  {
    name: 'addimm_h51_158',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x158',
    body: '62 51 158',
    expected: pinOf(0x62, [0x51, 0x158]),
  },
  {
    name: 'addimm_h52_158',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x158',
    body: '62 52 158',
    expected: pinOf(0x62, [0x52, 0x158]),
  },
  {
    name: 'subimm_h50_158',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x158',
    body: '61 50 158',
    expected: pinOf(0x61, [0x50, 0x158]),
  },
  {
    name: 'subimm_h51_158',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x158',
    body: '61 51 158',
    expected: pinOf(0x61, [0x51, 0x158]),
  },
  {
    name: 'subimm_h52_158',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x158',
    body: '61 52 158',
    expected: pinOf(0x61, [0x52, 0x158]),
  },
  {
    name: 'ldb_5060_160',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x160',
    body: '80 50 60 160',
    expected: pinOf(0x80, [0x50, 0x60, 0x160]),
  },
  {
    name: 'ldb_5160_160',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x160',
    body: '80 51 60 160',
    expected: pinOf(0x80, [0x51, 0x60, 0x160]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-64-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 485;
const SEL0 = 0x1eb;
const PIN = 'f9afff3e953337091fdaa161a919f6d92488d72c1f70687907395922a811ec42';
...
const log = `# parallel-batch-64 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-069 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-64-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
