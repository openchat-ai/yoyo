/**
 * parallel-batch-65 probe — scratch-only fresh picks after H_485..H_492.
 * Mirror parallel-batch-64-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_492. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 160 LDB triad + start 160 ADD/SUB triads + start 168 LDB.
const picks = [
  {
    name: 'ldb_5260_160',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x160',
    body: '80 52 60 160',
    expected: pinOf(0x80, [0x52, 0x60, 0x160]),
  },
  {
    name: 'addimm_h50_160',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x160',
    body: '62 50 160',
    expected: pinOf(0x62, [0x50, 0x160]),
  },
  {
    name: 'addimm_h51_160',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x160',
    body: '62 51 160',
    expected: pinOf(0x62, [0x51, 0x160]),
  },
  {
    name: 'addimm_h52_160',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x160',
    body: '62 52 160',
    expected: pinOf(0x62, [0x52, 0x160]),
  },
  {
    name: 'subimm_h50_160',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x160',
    body: '61 50 160',
    expected: pinOf(0x61, [0x50, 0x160]),
  },
  {
    name: 'subimm_h51_160',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x160',
    body: '61 51 160',
    expected: pinOf(0x61, [0x51, 0x160]),
  },
  {
    name: 'subimm_h52_160',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x160',
    body: '61 52 160',
    expected: pinOf(0x61, [0x52, 0x160]),
  },
  {
    name: 'ldb_5060_168',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x168',
    body: '80 50 60 168',
    expected: pinOf(0x80, [0x50, 0x60, 0x168]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-65-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 493;
const SEL0 = 0x1f3;
const PIN = '192ba67ac8bb814df865a108032dd1e9301c93c4e3fc89f44c8c4edfaf84791f';
...
const log = `# parallel-batch-65 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-070 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-65-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
