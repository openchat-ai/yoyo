/**
 * parallel-batch-89 probe — scratch-only fresh picks after H_677..H_684.
 * Mirror parallel-batch-88-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_684. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish deferred 208 ADD triad; start 208 SUB triad; start 210 LDB/ADD ladder.
const picks = [
  {
    name: 'addimm_h52_208',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x208',
    body: '62 52 208',
    expected: pinOf(0x62, [0x52, 0x208]),
  },
  {
    name: 'subimm_h50_208',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 50 208',
    expected: pinOf(0x61, [0x50, 0x208]),
  },
  {
    name: 'subimm_h51_208',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 51 208',
    expected: pinOf(0x61, [0x51, 0x208]),
  },
  {
    name: 'subimm_h52_208',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x208',
    body: '61 52 208',
    expected: pinOf(0x61, [0x52, 0x208]),
  },
  {
    name: 'ldb_5060_210',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x210',
    body: '80 50 60 210',
    expected: pinOf(0x80, [0x50, 0x60, 0x210]),
  },
  {
    name: 'ldb_5160_210',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x210',
    body: '80 51 60 210',
    expected: pinOf(0x80, [0x51, 0x60, 0x210]),
  },
  {
    name: 'ldb_5260_210',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x210',
    body: '80 52 60 210',
    expected: pinOf(0x80, [0x52, 0x60, 0x210]),
  },
  {
    name: 'addimm_h50_210',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x210',
    body: '62 50 210',
    expected: pinOf(0x62, [0x50, 0x210]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-89-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 685;
const SEL0 = 0x2b3;
const PIN = '0ef9611b50021d82d2c7870a29d1d4107164b7a3c586f41f5271a083fbdfec51';
...
const log = `# parallel-batch-89 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-094 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-89-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
