/**
 * parallel-batch-29 probe — scratch-only fresh picks after H_198..H_205.
 * Mirror parallel-batch-28-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_205. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
const picks = [
  {
    name: 'addimm_h51_58',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x58',
    body: '62 51 58',
    expected: pinOf(0x62, [0x51, 0x58]),
  },
  {
    name: 'addimm_h52_58',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x58',
    body: '62 52 58',
    expected: pinOf(0x62, [0x52, 0x58]),
  },
  {
    name: 'subimm_h50_50',
    opcode: '0x61 SUB-IMM',
  ...
  },
  {
    ...
    body: '61 52 50',
    expected: pinOf(0x61, [0x52, 0x50]),
  },
  {
    name: 'ldb_5260_78',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x78',
    body: '80 52 60 78',
    expected: pinOf(0x80, [0x52, 0x60, 0x78]),
  },
  {
    name: 'set_52_c0dec0de',
    opcode: '0x30 SET',
    args: 'slot=0x52 imm=0xC0DEC0DE',
    body: '30 52 C0DEC0DE',
    expected: pinOf(0x30, [0x52, 0xc0dec0de]),
  },
  {
    name: 'addimm_h50_60',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 50 60',
    expected: pinOf(0x62, [0x50, 0x60]),
  },
  {
    name: 'ldb_5060_80',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x80',
    body: '80 50 60 80',
    expected: pinOf(0x80, [0x50, 0x60, 0x80]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-29-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 206 + i;
    const sel = (0xd4 + i).toString(16).toUpperCase().padStart(2, '0');
...
const PIN = 'e531a0a8962e21ecce4f085df042195b60eb72b69f90d468f04cfeaa9c283588';

const log = `# parallel-batch-29 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-034 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${206 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-29-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
