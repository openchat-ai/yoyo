/**
 * parallel-batch-44 probe — scratch-only fresh picks after H_318..H_325.
 * Mirror parallel-batch-43-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_325. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// LDB oo=D8 → imm32 add (48 81 c0) → 26B.
// SUB-IMM/ADD-IMM imm=0xC8 (>127) → imm32 → 22B.
const picks = [
  {
    name: 'ldb_5060_d8',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0xD8',
    body: '80 50 60 D8',
    expected: pinOf(0x80, [0x50, 0x60, 0xd8]),
  },
  {
    name: 'ldb_5160_d8',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0xD8',
    body: '80 51 60 D8',
    expected: pinOf(0x80, [0x51, 0x60, 0xd8]),
  },
  {
    name: 'ldb_5260_d8',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0xD8',
    body: '80 52 60 D8',
    expected: pinOf(0x80, [0x52, 0x60, 0xd8]),
  },
  {
    name: 'addimm_h50_c8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0xC8',
    body: '62 50 C8',
    expected: pinOf(0x62, [0x50, 0xc8]),
  },
  {
    name: 'addimm_h51_c8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0xC8',
    body: '62 51 C8',
    expected: pinOf(0x62, [0x51, 0xc8]),
  },
  {
    name: 'addimm_h52_c8',
    opcode: '0x62 ADD-IMM',
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-44-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 326 + i;
    const sel = (0x14c + i).toString(16).toUpperCase();
...
const PIN = '69adc5a0b11c8f176687deff6753b2fa51b6611c3cd1193c79bf1143b7b4c957';

const log = `# parallel-batch-44 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-049 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${326 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-44-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
