/**
 * parallel-batch-59 probe — scratch-only fresh picks after H_437..H_444.
 * Mirror parallel-batch-58-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_444. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 130 SUB triad + start 138 LDB/ADD triads.
const picks = [
  {
    name: 'subimm_h52_130',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x130',
    body: '61 52 130',
    expected: pinOf(0x61, [0x52, 0x130]),
  },
  {
    name: 'ldb_5060_138',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x138',
    body: '80 50 60 138',
    expected: pinOf(0x80, [0x50, 0x60, 0x138]),
  },
  {
    name: 'ldb_5160_138',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x138',
    body: '80 51 60 138',
    expected: pinOf(0x80, [0x51, 0x60, 0x138]),
  },
  {
    name: 'ldb_5260_138',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x138',
    body: '80 52 60 138',
    expected: pinOf(0x80, [0x52, 0x60, 0x138]),
  },
  {
    name: 'addimm_h50_138',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x138',
    body: '62 50 138',
    expected: pinOf(0x62, [0x50, 0x138]),
  },
  {
    name: 'addimm_h51_138',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x138',
    body: '62 51 138',
    expected: pinOf(0x62, [0x51, 0x138]),
  },
  {
    name: 'addimm_h52_138',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x138',
    body: '62 52 138',
    expected: pinOf(0x62, [0x52, 0x138]),
  },
  {
    name: 'subimm_h50_138',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x138',
    body: '61 50 138',
    expected: pinOf(0x61, [0x50, 0x138]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-59-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 445;
const SEL0 = 0x1c3;
const PIN = 'd9aff9ed76e4f649fcee1c50496dd813e23690f73b35ce4cfc4e700ef466f276';
...
const log = `# parallel-batch-59 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-064 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-59-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
