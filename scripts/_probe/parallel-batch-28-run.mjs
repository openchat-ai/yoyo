/**
 * parallel-batch-28 probe — scratch-only fresh picks after H_190..H_197.
 * Mirror parallel-batch-27-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_197 and early yoyo.ty. Skipped LDB 50 60 70 (H_47).
...
const picks = [
  {
    name: 'addimm_h52_50',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x50',
    body: '62 52 50',
    expected: pinOf(0x62, [0x52, 0x50]),
  },
  {
    name: 'subimm_h50_48',
    opcode: '0x61 SUB-IMM',
  ...
  },
  {
    ...
    body: '61 52 48',
    expected: pinOf(0x61, [0x52, 0x48]),
  },
  {
    name: 'ldb_5060_78',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x78',
    body: '80 50 60 78',
    expected: pinOf(0x80, [0x50, 0x60, 0x78]),
  },
  {
    name: 'set_51_c0dec0de',
    opcode: '0x30 SET',
    args: 'slot=0x51 imm=0xC0DEC0DE',
    body: '30 51 C0DEC0DE',
    expected: pinOf(0x30, [0x51, 0xc0dec0de]),
  },
  {
    name: 'addimm_h50_58',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x58',
    body: '62 50 58',
    expected: pinOf(0x62, [0x50, 0x58]),
  },
  {
    name: 'subimm_h51_50',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 51 50',
    expected: pinOf(0x61, [0x51, 0x50]),
  },
  {
    name: 'ldb_5160_78',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x78',
    body: '80 51 60 78',
    expected: pinOf(0x80, [0x51, 0x60, 0x78]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-28-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 198 + i;
    const sel = (0xcc + i).toString(16).toUpperCase().padStart(2, '0');
...
const PIN = '0f0fce9a754e262914c8e2a78ca2558bd8af31ab0d532339f49018c2354cdac2';

const log = `# parallel-batch-28 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-033 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${198 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-28-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
