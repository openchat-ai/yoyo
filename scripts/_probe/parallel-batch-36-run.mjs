/**
 * parallel-batch-36 probe — scratch-only fresh picks after H_254..H_261.
 * Mirror parallel-batch-35-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_261. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// LDB oo≥0x80 → imm32 add (48 81 c0) → 26B.
// SUB-IMM/ADD-IMM imm=0x88/0x90 (>127) → imm32.
const picks = [
  {
    name: 'addimm_h52_88',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x88',
    body: '62 52 88',
    expected: pinOf(0x62, [0x52, 0x88]),
  },
  {
    name: 'subimm_h50_88',
    opcode: '0x61 SUB-IMM',
  ...
  },
  {
    ...
    args: 'slot=0x52 imm=0x88',
    body: '61 52 88',
    expected: pinOf(0x61, [0x52, 0x88]),
  },
  {
    name: 'ldb_5060_a8',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0xA8',
    body: '80 50 60 A8',
    expected: pinOf(0x80, [0x50, 0x60, 0xa8]),
  },
  {
    name: 'ldb_5160_a8',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0xA8',
    body: '80 51 60 A8',
    expected: pinOf(0x80, [0x51, 0x60, 0xa8]),
  },
  {
    name: 'ldb_5260_a8',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0xA8',
    body: '80 52 60 A8',
    expected: pinOf(0x80, [0x52, 0x60, 0xa8]),
  },
  {
    name: 'addimm_h50_90',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x90',
    body: '62 50 90',
    expected: pinOf(0x62, [0x50, 0x90]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-36-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 262 + i;
    const sel = (0x10c + i).toString(16).toUpperCase();
...
const PIN = '4cb656812b03c0fdb229b2d0d9278c479ab83b33d6cc7782e75f2397b0e165db';

const log = `# parallel-batch-36 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-041 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${262 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-36-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
