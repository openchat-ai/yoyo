/**
 * parallel-batch-51 probe — scratch-only fresh picks after H_374..H_381.
 * Mirror parallel-batch-50-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_381. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish F8 SUB triad; then LDB/ADD/SUB imm=100 (next after F8).
const picks = [
  {
    name: 'subimm_h52_f8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0xF8',
    body: '61 52 F8',
    expected: pinOf(0x61, [0x52, 0xf8]),
  },
  {
    name: 'ldb_5060_100',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x100',
    body: '80 50 60 100',
    expected: pinOf(0x80, [0x50, 0x60, 0x100]),
  },
  {
    name: 'ldb_5160_100',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x100',
    body: '80 51 60 100',
    expected: pinOf(0x80, [0x51, 0x60, 0x100]),
  },
  {
    name: 'ldb_5260_100',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x100',
    body: '80 52 60 100',
    expected: pinOf(0x80, [0x52, 0x60, 0x100]),
  },
  {
    name: 'addimm_h50_100',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x100',
    body: '62 50 100',
    expected: pinOf(0x62, [0x50, 0x100]),
  },
  {
    name: 'addimm_h51_100',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 51 100',
    expected: pinOf(0x62, [0x51, 0x100]),
  },
  {
    name: 'addimm_h52_100',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 52 100',
    expected: pinOf(0x62, [0x52, 0x100]),
  },
  {
    name: 'subimm_h50_100',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x100',
    body: '61 50 100',
    expected: pinOf(0x61, [0x50, 0x100]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-51-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 382;
const SEL0 = 0x184;
const PIN = '824207c608fe5d03e4bd1c3bca1f33aec844dd62f4bc66ca4a6877364538314b';
...
const log = `# parallel-batch-51 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-056 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-51-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
