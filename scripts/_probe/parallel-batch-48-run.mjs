/**
 * parallel-batch-48 probe — scratch-only fresh picks after H_350..H_357.
 * Mirror parallel-batch-47-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_357. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// SUB-IMM 52 E0 finishes E0 triad; ADD/SUB imm=E8 fresh; LDB oo=F0.
const picks = [
  {
    name: 'subimm_h52_e0',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 52 E0',
    expected: pinOf(0x61, [0x52, 0xe0]),
  },
  {
    name: 'addimm_h50_e8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0xE8',
    body: '62 50 E8',
    expected: pinOf(0x62, [0x50, 0xe8]),
  },
  {
    name: 'addimm_h51_e8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0xE8',
    body: '62 51 E8',
    expected: pinOf(0x62, [0x51, 0xe8]),
  },
  {
    name: 'addimm_h52_e8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0xE8',
    body: '62 52 E8',
    expected: pinOf(0x62, [0x52, 0xe8]),
  },
  {
    name: 'subimm_h50_e8',
    opcode: '0x61 SUB-IMM',
  ...
  },
  {
    ...
    args: 'slot=0x52 imm=0xE8',
    body: '61 52 E8',
    expected: pinOf(0x61, [0x52, 0xe8]),
  },
  {
    name: 'ldb_5060_f0',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0xF0',
    body: '80 50 60 F0',
    expected: pinOf(0x80, [0x50, 0x60, 0xf0]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-48-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 358;
const SEL0 = 0x16c;
const PIN = '86485f4822e891c4f11dbc5f181c43dc3f23d7ed779b61831f2426f2329e504d';
...
const log = `# parallel-batch-48 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-053 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-48-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
