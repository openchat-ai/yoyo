/**
 * parallel-batch-49 probe — scratch-only fresh picks after H_358..H_365.
 * Mirror parallel-batch-48-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_365. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// LDB 51/52 60 F0 finishes F0 triad after H_365; ADD/SUB imm=F0 fresh.
const picks = [
  {
    name: 'ldb_5160_f0',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0xF0',
    body: '80 51 60 F0',
    expected: pinOf(0x80, [0x51, 0x60, 0xf0]),
  },
  {
    name: 'ldb_5260_f0',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0xF0',
    body: '80 52 60 F0',
    expected: pinOf(0x80, [0x52, 0x60, 0xf0]),
  },
  {
    name: 'addimm_h50_f0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0xF0',
    body: '62 50 F0',
    expected: pinOf(0x62, [0x50, 0xf0]),
  },
  {
    name: 'addimm_h51_f0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0xF0',
    body: '62 51 F0',
    expected: pinOf(0x62, [0x51, 0xf0]),
  },
  {
    name: 'addimm_h52_f0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0xF0',
    body: '62 52 F0',
    expected: pinOf(0x62, [0x52, 0xf0]),
  },
  {
    name: 'subimm_h50_f0',
    opcode: '0x61 SUB-IMM',
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-49-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 366;
const SEL0 = 0x174;
const PIN = '13cb91ab1e1cc24d3f4b6d9a151a2e9a8d487556099cc030a189d6ac30554d9b';
...
const log = `# parallel-batch-49 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-054 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-49-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
