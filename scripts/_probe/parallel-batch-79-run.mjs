/**
 * parallel-batch-79 probe — scratch-only fresh picks after H_597..H_604.
 * Mirror parallel-batch-78-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_604. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Start deferred 1C0 SUB triad; start 1C8 LDB / ADD.
const picks = [
  {
    name: 'subimm_h50_1C0',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 50 1C0',
    expected: pinOf(0x61, [0x50, 0x1c0]),
  },
  {
    name: 'subimm_h51_1C0',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 51 1C0',
    expected: pinOf(0x61, [0x51, 0x1c0]),
  },
  {
    name: 'subimm_h52_1C0',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 52 1C0',
    expected: pinOf(0x61, [0x52, 0x1c0]),
  },
  {
    name: 'ldb_5060_1C8',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x1C8',
    body: '80 50 60 1C8',
    expected: pinOf(0x80, [0x50, 0x60, 0x1c8]),
  },
  {
    name: 'ldb_5160_1C8',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x1C8',
    body: '80 51 60 1C8',
    expected: pinOf(0x80, [0x51, 0x60, 0x1c8]),
  },
  {
    name: 'ldb_5260_1C8',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x1C8',
    body: '80 52 60 1C8',
    expected: pinOf(0x80, [0x52, 0x60, 0x1c8]),
  },
  {
    name: 'addimm_h50_1C8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x1C8',
    body: '62 50 1C8',
    expected: pinOf(0x62, [0x50, 0x1c8]),
  },
  {
    name: 'addimm_h51_1C8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x1C8',
    body: '62 51 1C8',
    expected: pinOf(0x62, [0x51, 0x1c8]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-79-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 605;
const SEL0 = 0x263;
const PIN = '9eafc9ce0376d389043b0e77ec2c1ff2bc44dda11b4fb8f6449cc4ea811798ac';
...
const log = `# parallel-batch-79 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-084 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-79-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
