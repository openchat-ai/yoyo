/**
 * parallel-batch-72 probe — scratch-only fresh picks after H_541..H_548.
 * Mirror parallel-batch-71-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_548. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish deferred 190 ADD triad + start 190 SUB + 198 LDB.
const picks = [
  {
    name: 'addimm_h51_190',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x190',
    body: '62 51 190',
    expected: pinOf(0x62, [0x51, 0x190]),
  },
  {
    name: 'addimm_h52_190',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x190',
    body: '62 52 190',
    expected: pinOf(0x62, [0x52, 0x190]),
  },
  {
    name: 'subimm_h50_190',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 50 190',
    expected: pinOf(0x61, [0x50, 0x190]),
  },
  {
    name: 'subimm_h51_190',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x190',
    body: '61 51 190',
    expected: pinOf(0x61, [0x51, 0x190]),
  },
  {
    name: 'subimm_h52_190',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x190',
    body: '61 52 190',
    expected: pinOf(0x61, [0x52, 0x190]),
  },
  {
    name: 'ldb_5060_198',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x198',
    body: '80 50 60 198',
    expected: pinOf(0x80, [0x50, 0x60, 0x198]),
  },
  {
    name: 'ldb_5160_198',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x198',
    body: '80 51 60 198',
    expected: pinOf(0x80, [0x51, 0x60, 0x198]),
  },
  {
    name: 'ldb_5260_198',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x198',
    body: '80 52 60 198',
    expected: pinOf(0x80, [0x52, 0x60, 0x198]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-72-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 549;
const SEL0 = 0x22b;
const PIN = '97ce84a29adb8c400408d7fec9d2d58a820766a61c18068b1b61eac59946e2b0';
...
const log = `# parallel-batch-72 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-077 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-72-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
