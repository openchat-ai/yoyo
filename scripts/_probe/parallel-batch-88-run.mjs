/**
 * parallel-batch-88 probe — scratch-only fresh picks after H_669..H_676.
 * Mirror parallel-batch-87-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_676. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish deferred 200 SUB triad; start 208 LDB/ADD ladder.
const picks = [
  {
    name: 'subimm_h50_200',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 50 200',
    expected: pinOf(0x61, [0x50, 0x200]),
  },
  {
    name: 'subimm_h51_200',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 51 200',
    expected: pinOf(0x61, [0x51, 0x200]),
  },
  {
    name: 'subimm_h52_200',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 52 200',
    expected: pinOf(0x61, [0x52, 0x200]),
  },
  {
    name: 'ldb_5060_208',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x208',
    body: '80 50 60 208',
    expected: pinOf(0x80, [0x50, 0x60, 0x208]),
  },
  {
    name: 'ldb_5160_208',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x208',
    body: '80 51 60 208',
    expected: pinOf(0x80, [0x51, 0x60, 0x208]),
  },
  {
    name: 'ldb_5260_208',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x208',
    body: '80 52 60 208',
    expected: pinOf(0x80, [0x52, 0x60, 0x208]),
  },
  {
    name: 'addimm_h50_208',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x208',
    body: '62 50 208',
    expected: pinOf(0x62, [0x50, 0x208]),
  },
  {
    name: 'addimm_h51_208',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x208',
    body: '62 51 208',
    expected: pinOf(0x62, [0x51, 0x208]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-88-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 677;
const SEL0 = 0x2ab;
const PIN = '04656bbbbb152b5402bd76daa324a51a7f68477df3b3615827ef88aa2907978b';
...
const log = `# parallel-batch-88 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-093 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-88-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
