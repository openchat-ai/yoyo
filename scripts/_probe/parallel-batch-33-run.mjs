/**
 * parallel-batch-33 probe — scratch-only fresh picks after H_230..H_237.
 * Mirror parallel-batch-32-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_237. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
...
const picks = [
  {
    name: 'ldb_5260_90',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x90',
    body: '80 52 60 90',
    expected: pinOf(0x80, [0x52, 0x60, 0x90]),
  },
  {
    name: 'subimm_h50_70',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x70',
    body: '61 50 70',
    expected: pinOf(0x61, [0x50, 0x70]),
  },
  {
    name: 'subimm_h51_70',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x70',
    body: '61 51 70',
    expected: pinOf(0x61, [0x51, 0x70]),
  },
  {
    name: 'subimm_h52_70',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x70',
    body: '61 52 70',
    expected: pinOf(0x61, [0x52, 0x70]),
  },
  {
    name: 'addimm_h50_78',
    opcode: '0x62 ADD-IMM',
  ...
  },
  {
    ...
    body: '62 52 78',
    expected: pinOf(0x62, [0x52, 0x78]),
  },
  {
    name: 'ldb_5060_98',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x98',
    body: '80 50 60 98',
    expected: pinOf(0x80, [0x50, 0x60, 0x98]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-33-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 238 + i;
    const sel = (0xf4 + i).toString(16).toUpperCase().padStart(2, '0');
...
const PIN = 'aa95228f49b6131c88315b4af43e02b76e8c67070322eab4c200944e839a99fa';

const log = `# parallel-batch-33 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-038 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${238 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-33-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
