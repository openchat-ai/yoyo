/**
 * parallel-batch-58 probe — scratch-only fresh picks after H_430..H_436.
 * Mirror parallel-batch-57-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_436. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Start 130 LDB/ADD triads + SUB 50/51.
const picks = [
  {
    name: 'ldb_5060_130',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x130',
    body: '80 50 60 130',
    expected: pinOf(0x80, [0x50, 0x60, 0x130]),
  },
  {
    name: 'ldb_5160_130',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x130',
    body: '80 51 60 130',
    expected: pinOf(0x80, [0x51, 0x60, 0x130]),
  },
  {
    name: 'ldb_5260_130',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x130',
    body: '80 52 60 130',
    expected: pinOf(0x80, [0x52, 0x60, 0x130]),
  },
  {
    name: 'addimm_h50_130',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x130',
    body: '62 50 130',
    expected: pinOf(0x62, [0x50, 0x130]),
  },
  {
    name: 'addimm_h51_130',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x130',
    body: '62 51 130',
    expected: pinOf(0x62, [0x51, 0x130]),
  },
  {
    name: 'addimm_h52_130',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 52 130',
    expected: pinOf(0x62, [0x52, 0x130]),
  },
  {
    name: 'subimm_h50_130',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x130',
    body: '61 50 130',
    expected: pinOf(0x61, [0x50, 0x130]),
  },
  {
    name: 'subimm_h51_130',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x130',
    body: '61 51 130',
    expected: pinOf(0x61, [0x51, 0x130]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-58-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 437;
const SEL0 = 0x1bb;
const PIN = 'f4fa77a59520fda01683d3ceffe44de6886ba77752450ffbb0947e0ba15f0d96';
...
const log = `# parallel-batch-58 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-063 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-58-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
