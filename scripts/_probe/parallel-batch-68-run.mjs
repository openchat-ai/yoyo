/**
 * parallel-batch-68 probe — scratch-only fresh picks after H_509..H_516.
 * Mirror parallel-batch-67-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_516. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 170 SUB triad + start 178 LDB/ADD + start 178 SUB (50).
const picks = [
  {
    name: 'subimm_h52_170',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x170',
    body: '61 52 170',
    expected: pinOf(0x61, [0x52, 0x170]),
  },
  {
    name: 'ldb_5060_178',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x178',
    body: '80 50 60 178',
    expected: pinOf(0x80, [0x50, 0x60, 0x178]),
  },
  {
    name: 'ldb_5160_178',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x178',
    body: '80 51 60 178',
    expected: pinOf(0x80, [0x51, 0x60, 0x178]),
  },
  {
    name: 'ldb_5260_178',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x178',
    body: '80 52 60 178',
    expected: pinOf(0x80, [0x52, 0x60, 0x178]),
  },
  {
    name: 'addimm_h50_178',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x178',
    body: '62 50 178',
    expected: pinOf(0x62, [0x50, 0x178]),
  },
  {
    name: 'addimm_h51_178',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 51 178',
    expected: pinOf(0x62, [0x51, 0x178]),
  },
  {
    name: 'addimm_h52_178',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 52 178',
    expected: pinOf(0x62, [0x52, 0x178]),
  },
  {
    name: 'subimm_h50_178',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x178',
    body: '61 50 178',
    expected: pinOf(0x61, [0x50, 0x178]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-68-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 517;
const SEL0 = 0x20b;
const PIN = '1a6cb44aa28367d25d6727eec5206e5895c3c948be080a60dcadb7d853bc8bac';
...
const log = `# parallel-batch-68 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-073 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-68-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
