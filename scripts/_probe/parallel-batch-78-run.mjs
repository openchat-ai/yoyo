/**
 * parallel-batch-78 probe — scratch-only fresh picks after H_589..H_596.
 * Mirror parallel-batch-77-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_596. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish deferred 1B8 SUB triad; start 1C0 LDB / ADD.
const picks = [
  {
    name: 'subimm_h51_1B8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x1B8',
    body: '61 51 1B8',
    expected: pinOf(0x61, [0x51, 0x1b8]),
  },
  {
    name: 'subimm_h52_1B8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x1B8',
    body: '61 52 1B8',
    expected: pinOf(0x61, [0x52, 0x1b8]),
  },
  {
    name: 'ldb_5060_1C0',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x1C0',
    body: '80 50 60 1C0',
    expected: pinOf(0x80, [0x50, 0x60, 0x1c0]),
  },
  {
    name: 'ldb_5160_1C0',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x1C0',
    body: '80 51 60 1C0',
    expected: pinOf(0x80, [0x51, 0x60, 0x1c0]),
  },
  {
    name: 'ldb_5260_1C0',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x1C0',
    body: '80 52 60 1C0',
    expected: pinOf(0x80, [0x52, 0x60, 0x1c0]),
  },
  {
    name: 'addimm_h50_1C0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x1C0',
    body: '62 50 1C0',
    expected: pinOf(0x62, [0x50, 0x1c0]),
  },
  {
    name: 'addimm_h51_1C0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x1C0',
    body: '62 51 1C0',
    expected: pinOf(0x62, [0x51, 0x1c0]),
  },
  {
    name: 'addimm_h52_1C0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x1C0',
    body: '62 52 1C0',
    expected: pinOf(0x62, [0x52, 0x1c0]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-78-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 597;
const SEL0 = 0x25b;
const PIN = '45dff031e2acfa0ee40a932a4bca8709747e45bb1ac19f622fe0c477c4fe4a44';
...
const log = `# parallel-batch-78 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-083 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-78-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
