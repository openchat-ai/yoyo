/**
 * parallel-batch-26 probe — scratch-only fresh picks after H_174..H_181.
 * Mirror parallel-batch-25-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_181 and early yoyo.ty. Skipped early-dup INC/DEC 50,
// GET/ORV/ADDV/SUBV/IMUL 50 51, LDB 50 60 60 (H_46). Skip D-1/D-2/D-3 / MEMCPY.
// No AND/XOR. Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
const picks = [
  {
    name: 'set_52_deadc0de',
    opcode: '0x30 SET',
    args: 'slot=0x52 imm=0xDEADC0DE',
    body: '30 52 DEADC0DE',
    expected: pinOf(0x30, [0x52, 0xdeadc0de]),
  },
  {
    name: 'ldb_5060_68',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x68',
    body: '80 50 60 68',
    expected: pinOf(0x80, [0x50, 0x60, 0x68]),
  },
  {
    name: 'ldb_5160_68',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x68',
    body: '80 51 60 68',
    expected: pinOf(0x80, [0x51, 0x60, 0x68]),
  },
  {
    name: 'ldb_5260_68',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x68',
    body: '80 52 60 68',
    expected: pinOf(0x80, [0x52, 0x60, 0x68]),
  },
  {
    name: 'addimm_h50_48',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x48',
    body: '62 50 48',
    expected: pinOf(0x62, [0x50, 0x48]),
  },
  {
    name: 'addimm_h51_48',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 51 48',
    expected: pinOf(0x62, [0x51, 0x48]),
  },
  {
    name: 'subimm_h50_40',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x40',
    body: '61 50 40',
    expected: pinOf(0x61, [0x50, 0x40]),
  },
  {
    name: 'subimm_h51_40',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x40',
    body: '61 51 40',
    expected: pinOf(0x61, [0x51, 0x40]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-26-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 182 + i;
    const sel = (0xbc + i).toString(16).toUpperCase().padStart(2, '0');
...
const PIN = 'dc10b2bd70d2232bc015d3a87c88a02d58d5eaffd5ae572fd219dd84094db127';

const log = `# parallel-batch-26 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-031 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${182 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-26-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
