/**
 * parallel-batch-82 probe — scratch-only fresh picks after H_621..H_628.
 * Mirror parallel-batch-81-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_628. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Start 1D8 ADD; start 1D8 SUB; start 1E0 LDB (2 of triad).
const picks = [
  {
    name: 'addimm_h50_1D8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x1D8',
    body: '62 50 1D8',
    expected: pinOf(0x62, [0x50, 0x1d8]),
  },
  {
    name: 'addimm_h51_1D8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x1D8',
    body: '62 51 1D8',
    expected: pinOf(0x62, [0x51, 0x1d8]),
  },
  {
    name: 'addimm_h52_1D8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x1D8',
    body: '62 52 1D8',
    expected: pinOf(0x62, [0x52, 0x1d8]),
  },
  {
    name: 'subimm_h50_1D8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x1D8',
    body: '61 50 1D8',
    expected: pinOf(0x61, [0x50, 0x1d8]),
  },
  {
    name: 'subimm_h51_1D8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x1D8',
    body: '61 51 1D8',
    expected: pinOf(0x61, [0x51, 0x1d8]),
  },
  {
    name: 'subimm_h52_1D8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x1D8',
    body: '61 52 1D8',
    expected: pinOf(0x61, [0x52, 0x1d8]),
  },
  {
    name: 'ldb_5060_1E0',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x1E0',
    body: '80 50 60 1E0',
    expected: pinOf(0x80, [0x50, 0x60, 0x1e0]),
  },
  {
    name: 'ldb_5160_1E0',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x1E0',
    body: '80 51 60 1E0',
    expected: pinOf(0x80, [0x51, 0x60, 0x1e0]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-82-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 629;
const SEL0 = 0x27b;
const PIN = 'db550629db78a974cd83bec8db879fec415cd6fe37c94b35f57ce10a6917010d';
...
const log = `# parallel-batch-82 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-087 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-82-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
