/**
 * parallel-batch-22 probe — scratch-only fresh picks after H_142..H_149.
 * Mirror parallel-batch-21-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_149. Skip early-dup INC/DEC 50, GET/ORV/ADDV/SUBV 50 51,
// IMUL 50 51, LDB 50 60 40 (H_43). Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
const picks = [
  {
    name: 'set_50_feedc0de',
    opcode: '0x30 SET',
    args: 'slot=0x50 imm=0xFEEDC0DE',
    body: '30 50 FEEDC0DE',
    expected: pinOf(0x30, [0x50, 0xfeedc0de]),
  },
  {
    name: 'addimm_h50_32',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x32',
    body: '62 50 32',
    expected: pinOf(0x62, [0x50, 0x32]),
  },
  {
    name: 'subimm_h52_28',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 52 28',
    expected: pinOf(0x61, [0x52, 0x28]),
  },
  {
    name: 'ldb_5060_48',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x48',
    body: '80 50 60 48',
    expected: pinOf(0x80, [0x50, 0x60, 0x48]),
  },
  {
    name: 'ldb_5160_48',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x48',
    body: '80 51 60 48',
    expected: pinOf(0x80, [0x51, 0x60, 0x48]),
  },
  {
    name: 'ldb_5260_48',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x48',
    body: '80 52 60 48',
    expected: pinOf(0x80, [0x52, 0x60, 0x48]),
  },
  {
    name: 'addimm_h51_32',
    opcode: '0x62 ADD-IMM',
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-22-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 150 + i;
    const sel = (0x9c + i).toString(16).toUpperCase().padStart(2, '0');
...
const PIN = '2a14beec0f08ffdfd64656bc2230706c4ec1928a697bf00f3905ff724c4d28f2';

const log = `# parallel-batch-22 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-027 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${150 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-22-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
