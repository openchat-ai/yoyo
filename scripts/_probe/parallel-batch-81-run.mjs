/**
 * parallel-batch-81 probe — scratch-only fresh picks after H_613..H_620.
 * Mirror parallel-batch-80-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_620. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 1D0 ADD; start 1D0 SUB; start 1D8 LDB.
const picks = [
  {
    name: 'addimm_h51_1D0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x1D0',
    body: '62 51 1D0',
    expected: pinOf(0x62, [0x51, 0x1d0]),
  },
  {
    name: 'addimm_h52_1D0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x1D0',
    body: '62 52 1D0',
    expected: pinOf(0x62, [0x52, 0x1d0]),
  },
  {
    name: 'subimm_h50_1D0',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 50 1D0',
    expected: pinOf(0x61, [0x50, 0x1d0]),
  },
  {
    name: 'subimm_h51_1D0',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x1D0',
    body: '61 51 1D0',
    expected: pinOf(0x61, [0x51, 0x1d0]),
  },
  {
    name: 'subimm_h52_1D0',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x1D0',
    body: '61 52 1D0',
    expected: pinOf(0x61, [0x52, 0x1d0]),
  },
  {
    name: 'ldb_5060_1D8',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x1D8',
    body: '80 50 60 1D8',
    expected: pinOf(0x80, [0x50, 0x60, 0x1d8]),
  },
  {
    name: 'ldb_5160_1D8',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x1D8',
    body: '80 51 60 1D8',
    expected: pinOf(0x80, [0x51, 0x60, 0x1d8]),
  },
  {
    name: 'ldb_5260_1D8',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x1D8',
    body: '80 52 60 1D8',
    expected: pinOf(0x80, [0x52, 0x60, 0x1d8]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-81-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 621;
const SEL0 = 0x273;
const PIN = '9546a03ee5ac5d5254a4d887560694622666ef2cfc3a6035a937c978dfd5ee67';
...
const log = `# parallel-batch-81 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-086 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-81-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
