/**
 * parallel-batch-46 probe — scratch-only fresh picks after H_334..H_341.
 * Mirror parallel-batch-45-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_341. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// LDB 51/52 E0 finishes E0 triad after H_341; ADD/SUB D8 fresh imm after D0.
const picks = [
  {
    name: 'ldb_5160_e0',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0xE0',
    body: '80 51 60 E0',
    expected: pinOf(0x80, [0x51, 0x60, 0xe0]),
  },
  {
    name: 'ldb_5260_e0',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0xE0',
    body: '80 52 60 E0',
    expected: pinOf(0x80, [0x52, 0x60, 0xe0]),
  },
  {
    name: 'addimm_h50_d8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0xD8',
    body: '62 50 D8',
    expected: pinOf(0x62, [0x50, 0xd8]),
  },
  {
    name: 'addimm_h51_d8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0xD8',
    body: '62 51 D8',
    expected: pinOf(0x62, [0x51, 0xd8]),
  },
  {
    name: 'addimm_h52_d8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0xD8',
    body: '62 52 D8',
    expected: pinOf(0x62, [0x52, 0xd8]),
  },
  {
    name: 'subimm_h50_d8',
    opcode: '0x61 SUB-IMM',
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-46-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 342;
const SEL0 = 0x15c;
const PIN = 'ee5b881e34301f79f6c647181243709ea5ccfdbf03a2088c7d44b1de98d91b4f';
...
const log = `# parallel-batch-46 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-051 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-46-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
