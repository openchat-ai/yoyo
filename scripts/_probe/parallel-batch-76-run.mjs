/**
 * parallel-batch-76 probe — scratch-only fresh picks after H_573..H_580.
 * Mirror parallel-batch-75-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_580. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Start deferred 1B0 LDB / ADD / SUB triads (SUB 52 deferred to next).
const picks = [
  {
    name: 'ldb_5060_1B0',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x1B0',
    body: '80 50 60 1B0',
    expected: pinOf(0x80, [0x50, 0x60, 0x1b0]),
  },
  {
    name: 'ldb_5160_1B0',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x1B0',
    body: '80 51 60 1B0',
    expected: pinOf(0x80, [0x51, 0x60, 0x1b0]),
  },
  {
    name: 'ldb_5260_1B0',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x1B0',
    body: '80 52 60 1B0',
    expected: pinOf(0x80, [0x52, 0x60, 0x1b0]),
  },
  {
    name: 'addimm_h50_1B0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x1B0',
    body: '62 50 1B0',
    expected: pinOf(0x62, [0x50, 0x1b0]),
  },
  {
    name: 'addimm_h51_1B0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x1B0',
    body: '62 51 1B0',
    expected: pinOf(0x62, [0x51, 0x1b0]),
  },
  {
    name: 'addimm_h52_1B0',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 52 1B0',
    expected: pinOf(0x62, [0x52, 0x1b0]),
  },
  {
    name: 'subimm_h50_1B0',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x1B0',
    body: '61 50 1B0',
    expected: pinOf(0x61, [0x50, 0x1b0]),
  },
  {
    name: 'subimm_h51_1B0',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x1B0',
    body: '61 51 1B0',
    expected: pinOf(0x61, [0x51, 0x1b0]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-76-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 581;
const SEL0 = 0x24b;
const PIN = '267c611dbb648db15251e6e6ee8a52287434680892e9f2ad290fd161eb2b916c';
...
const log = `# parallel-batch-76 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-081 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-76-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
