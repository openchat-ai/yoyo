/**
 * parallel-batch-77 probe — scratch-only fresh picks after H_581..H_588.
 * Mirror parallel-batch-76-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_588. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish deferred 1B0 SUB triad; start 1B8 LDB / ADD / SUB.
const picks = [
  {
    name: 'subimm_h52_1B0',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x1B0',
    body: '61 52 1B0',
    expected: pinOf(0x61, [0x52, 0x1b0]),
  },
  {
    name: 'ldb_5060_1B8',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x1B8',
    body: '80 50 60 1B8',
    expected: pinOf(0x80, [0x50, 0x60, 0x1b8]),
  },
  {
    name: 'ldb_5160_1B8',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x1B8',
    body: '80 51 60 1B8',
    expected: pinOf(0x80, [0x51, 0x60, 0x1b8]),
  },
  {
    name: 'ldb_5260_1B8',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x1B8',
    body: '80 52 60 1B8',
    expected: pinOf(0x80, [0x52, 0x60, 0x1b8]),
  },
  {
    name: 'addimm_h50_1B8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x1B8',
    body: '62 50 1B8',
    expected: pinOf(0x62, [0x50, 0x1b8]),
  },
  {
    name: 'addimm_h51_1B8',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 51 1B8',
    expected: pinOf(0x62, [0x51, 0x1b8]),
  },
  {
    name: 'addimm_h52_1B8',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 52 1B8',
    expected: pinOf(0x62, [0x52, 0x1b8]),
  },
  {
    name: 'subimm_h50_1B8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x1B8',
    body: '61 50 1B8',
    expected: pinOf(0x61, [0x50, 0x1b8]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-77-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 589;
const SEL0 = 0x253;
const PIN = '05a3a9c6693fa65c20f47a3eab1bc536c5e5fe0a168381faf0cf72330ca58056';
...
const log = `# parallel-batch-77 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-082 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-77-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
