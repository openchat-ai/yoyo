/**
 * parallel-batch-84 probe — scratch-only fresh picks after H_637..H_644.
 * Mirror parallel-batch-83-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_644. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 1E8 LDB; start 1E8 ADD; start 1E8 SUB.
const picks = [
  {
    name: 'ldb_5160_1E8',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x1E8',
    body: '80 51 60 1E8',
    expected: pinOf(0x80, [0x51, 0x60, 0x1e8]),
  },
  {
    name: 'ldb_5260_1E8',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x1E8',
    body: '80 52 60 1E8',
    expected: pinOf(0x80, [0x52, 0x60, 0x1e8]),
  },
  {
    name: 'addimm_h50_1E8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x1E8',
    body: '62 50 1E8',
    expected: pinOf(0x62, [0x50, 0x1e8]),
  },
  {
    name: 'addimm_h51_1E8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x1E8',
    body: '62 51 1E8',
    expected: pinOf(0x62, [0x51, 0x1e8]),
  },
  {
    name: 'addimm_h52_1E8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x1E8',
    body: '62 52 1E8',
    expected: pinOf(0x62, [0x52, 0x1e8]),
  },
  {
    name: 'subimm_h50_1E8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x1E8',
    body: '61 50 1E8',
    expected: pinOf(0x61, [0x50, 0x1e8]),
  },
  {
    name: 'subimm_h51_1E8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x1E8',
    body: '61 51 1E8',
    expected: pinOf(0x61, [0x51, 0x1e8]),
  },
  {
    name: 'subimm_h52_1E8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x1E8',
    body: '61 52 1E8',
    expected: pinOf(0x61, [0x52, 0x1e8]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-84-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 645;
const SEL0 = 0x28b;
const PIN = 'e8500277650750c55bc94ec1a9c5e0277367daa257b09371e33f569a8d46c129';
...
const log = `# parallel-batch-84 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-089 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-84-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
