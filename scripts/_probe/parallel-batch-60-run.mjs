/**
 * parallel-batch-60 probe — scratch-only fresh picks after H_445..H_452.
 * Mirror parallel-batch-59-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_452. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 138 SUB triad + start 140 LDB/ADD triads.
const picks = [
  {
    name: 'subimm_h51_138',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 51 138',
    expected: pinOf(0x61, [0x51, 0x138]),
  },
  {
    name: 'subimm_h52_138',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 52 138',
    expected: pinOf(0x61, [0x52, 0x138]),
  },
  {
    name: 'ldb_5060_140',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x140',
    body: '80 50 60 140',
    expected: pinOf(0x80, [0x50, 0x60, 0x140]),
  },
  {
    name: 'ldb_5160_140',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x140',
    body: '80 51 60 140',
    expected: pinOf(0x80, [0x51, 0x60, 0x140]),
  },
  {
    name: 'ldb_5260_140',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x140',
    body: '80 52 60 140',
    expected: pinOf(0x80, [0x52, 0x60, 0x140]),
  },
  {
    name: 'addimm_h50_140',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x140',
    body: '62 50 140',
    expected: pinOf(0x62, [0x50, 0x140]),
  },
  {
    name: 'addimm_h51_140',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x140',
    body: '62 51 140',
    expected: pinOf(0x62, [0x51, 0x140]),
  },
  {
    name: 'addimm_h52_140',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x140',
    body: '62 52 140',
    expected: pinOf(0x62, [0x52, 0x140]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-60-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 453;
const SEL0 = 0x1cb;
const PIN = 'b84d7f1b4bb1d8eefeca1832f12c3f7380658897813b1a321f2b75b27187258e';
...
const log = `# parallel-batch-60 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-065 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-60-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
