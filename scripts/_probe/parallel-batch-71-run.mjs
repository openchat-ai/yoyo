/**
 * parallel-batch-71 probe — scratch-only fresh picks after H_533..H_540.
 * Mirror parallel-batch-70-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_540. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish deferred 188 ADD triad + start 188 SUB + 190 LDB/ADD.
const picks = [
  {
    name: 'addimm_h52_188',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x188',
    body: '62 52 188',
    expected: pinOf(0x62, [0x52, 0x188]),
  },
  {
    name: 'subimm_h50_188',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 50 188',
    expected: pinOf(0x61, [0x50, 0x188]),
  },
  {
    name: 'subimm_h51_188',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 51 188',
    expected: pinOf(0x61, [0x51, 0x188]),
  },
  {
    name: 'subimm_h52_188',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x188',
    body: '61 52 188',
    expected: pinOf(0x61, [0x52, 0x188]),
  },
  {
    name: 'ldb_5060_190',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x190',
    body: '80 50 60 190',
    expected: pinOf(0x80, [0x50, 0x60, 0x190]),
  },
  {
    name: 'ldb_5160_190',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x190',
    body: '80 51 60 190',
    expected: pinOf(0x80, [0x51, 0x60, 0x190]),
  },
  {
    name: 'ldb_5260_190',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x190',
    body: '80 52 60 190',
    expected: pinOf(0x80, [0x52, 0x60, 0x190]),
  },
  {
    name: 'addimm_h50_190',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x190',
    body: '62 50 190',
    expected: pinOf(0x62, [0x50, 0x190]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-71-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 541;
const SEL0 = 0x223;
const PIN = 'ebbc6d765fcc0fcdc045848e93a3839d47ffdf287646adb781170a66d80690be';
...
const log = `# parallel-batch-71 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-076 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-71-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
