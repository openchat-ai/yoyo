/**
 * parallel-batch-75 probe — scratch-only fresh picks after H_565..H_572.
 * Mirror parallel-batch-74-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_572. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish deferred 1A8 LDB triad + start 1A8 ADD/SUB triads.
const picks = [
  {
    name: 'ldb_5160_1A8',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x1A8',
    body: '80 51 60 1A8',
    expected: pinOf(0x80, [0x51, 0x60, 0x1a8]),
  },
  {
    name: 'ldb_5260_1A8',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x1A8',
    body: '80 52 60 1A8',
    expected: pinOf(0x80, [0x52, 0x60, 0x1a8]),
  },
  {
    name: 'addimm_h50_1A8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x1A8',
    body: '62 50 1A8',
    expected: pinOf(0x62, [0x50, 0x1a8]),
  },
  {
    name: 'addimm_h51_1A8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x1A8',
    body: '62 51 1A8',
    expected: pinOf(0x62, [0x51, 0x1a8]),
  },
  {
    name: 'addimm_h52_1A8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x1A8',
    body: '62 52 1A8',
    expected: pinOf(0x62, [0x52, 0x1a8]),
  },
  {
    name: 'subimm_h50_1A8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x1A8',
    body: '61 50 1A8',
    expected: pinOf(0x61, [0x50, 0x1a8]),
  },
  {
    name: 'subimm_h51_1A8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x1A8',
    body: '61 51 1A8',
    expected: pinOf(0x61, [0x51, 0x1a8]),
  },
  {
    name: 'subimm_h52_1A8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x1A8',
    body: '61 52 1A8',
    expected: pinOf(0x61, [0x52, 0x1a8]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-75-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 573;
const SEL0 = 0x243;
const PIN = 'e255cd93a26ec455cc4def0ceb38c1cfc93bcb1ec7476f9e57ecd062d1be065a';
...
const log = `# parallel-batch-75 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-080 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-75-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
