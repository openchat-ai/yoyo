/**
 * parallel-batch-73 probe — scratch-only fresh picks after H_549..H_556.
 * Mirror parallel-batch-72-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_556. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Start deferred 198 ADD/SUB triads + start 1A0 LDB triad (2 of 3).
const picks = [
  {
    name: 'addimm_h50_198',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x198',
    body: '62 50 198',
    expected: pinOf(0x62, [0x50, 0x198]),
  },
  {
    name: 'addimm_h51_198',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x198',
    body: '62 51 198',
    expected: pinOf(0x62, [0x51, 0x198]),
  },
  {
    name: 'addimm_h52_198',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x198',
    body: '62 52 198',
    expected: pinOf(0x62, [0x52, 0x198]),
  },
  {
    name: 'subimm_h50_198',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x198',
    body: '61 50 198',
    expected: pinOf(0x61, [0x50, 0x198]),
  },
  {
    name: 'subimm_h51_198',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x198',
    body: '61 51 198',
    expected: pinOf(0x61, [0x51, 0x198]),
  },
  {
    name: 'subimm_h52_198',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x198',
    body: '61 52 198',
    expected: pinOf(0x61, [0x52, 0x198]),
  },
  {
    name: 'ldb_5060_1A0',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x1A0',
    body: '80 50 60 1A0',
    expected: pinOf(0x80, [0x50, 0x60, 0x1a0]),
  },
  {
    name: 'ldb_5160_1A0',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x1A0',
    body: '80 51 60 1A0',
    expected: pinOf(0x80, [0x51, 0x60, 0x1a0]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-73-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 557;
const SEL0 = 0x233;
const PIN = '4c42576df4f80a8d3f4e57074fb4fc081bc16d37c9638b9fd0659ddae86fd42b';
...
const log = `# parallel-batch-73 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-078 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-73-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
