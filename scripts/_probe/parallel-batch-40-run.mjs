/**
 * parallel-batch-40 probe — scratch-only fresh picks after H_286..H_293.
 * Mirror parallel-batch-39-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_293. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// LDB oo≥0x80 → imm32 add (48 81 c0) → 26B.
// SUB-IMM/ADD-IMM imm=0xA8 (>127) → imm32 → 22B.
const picks = [
  {
    name: 'ldb_5160_c0',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0xC0',
    body: '80 51 60 C0',
    expected: pinOf(0x80, [0x51, 0x60, 0xc0]),
  },
  {
    name: 'ldb_5260_c0',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0xC0',
    body: '80 52 60 C0',
    expected: pinOf(0x80, [0x52, 0x60, 0xc0]),
  },
  {
    name: 'addimm_h50_a8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0xA8',
    body: '62 50 A8',
    expected: pinOf(0x62, [0x50, 0xa8]),
  },
  {
    name: 'addimm_h51_a8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0xA8',
    body: '62 51 A8',
    expected: pinOf(0x62, [0x51, 0xa8]),
  },
  {
    name: 'addimm_h52_a8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0xA8',
    body: '62 52 A8',
    expected: pinOf(0x62, [0x52, 0xa8]),
  },
  {
    name: 'subimm_h50_a8',
    opcode: '0x61 SUB-IMM',
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-40-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 294 + i;
    const sel = (0x12c + i).toString(16).toUpperCase();
...
const PIN = '8c80a6fa783440b2ef724beb1860f295c81cde46c53f35d0cdcc40ff8798519c';

const log = `# parallel-batch-40 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-045 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${294 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-40-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
