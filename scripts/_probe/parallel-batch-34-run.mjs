/**
 * parallel-batch-34 probe — scratch-only fresh picks after H_238..H_245.
 * Mirror parallel-batch-33-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_245. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// LDB oo≥0x80 → imm32 add (48 81 c0) → 26B.
// ADD-IMM imm=80 (≤127) may use imm8.
const picks = [
  {
    name: 'ldb_5160_98',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x98',
    body: '80 51 60 98',
    expected: pinOf(0x80, [0x51, 0x60, 0x98]),
  },
  {
    name: 'ldb_5260_98',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x98',
    body: '80 52 60 98',
    expected: pinOf(0x80, [0x52, 0x60, 0x98]),
  },
  {
    name: 'subimm_h50_78',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x78',
    body: '61 50 78',
    expected: pinOf(0x61, [0x50, 0x78]),
  },
  {
    name: 'subimm_h51_78',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x78',
    body: '61 51 78',
    expected: pinOf(0x61, [0x51, 0x78]),
  },
  {
    name: 'subimm_h52_78',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x78',
    body: '61 52 78',
    expected: pinOf(0x61, [0x52, 0x78]),
  },
  {
    name: 'addimm_h50_80',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x80',
    body: '62 50 80',
    expected: pinOf(0x62, [0x50, 0x80]),
  },
  {
    name: 'addimm_h51_80',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x80',
    body: '62 51 80',
    expected: pinOf(0x62, [0x51, 0x80]),
  },
  {
    name: 'addimm_h52_80',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x80',
    body: '62 52 80',
    expected: pinOf(0x62, [0x52, 0x80]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-34-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 246 + i;
    const sel = (0xfc + i).toString(16).toUpperCase().padStart(2, '0');
...
const PIN = 'cc64da680d967e6b17ebc9767a74dd670251df82f614d0c72df1240126c05642';

const log = `# parallel-batch-34 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-039 Relock: \`${PIN}\`.
Handlers before consolidate = 252 (H_00..H_245). Next selectors 0xFC.. for H_246.. if all serialize.
(0xFF RET is opcode namespace only — HANDLER labels OK.)
...
${pass.map((r, i) => `  - H_${246 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-34-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
