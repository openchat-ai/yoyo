/**
 * parallel-batch-85 probe — scratch-only fresh picks after H_645..H_652.
 * Mirror parallel-batch-84-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_652. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Start deferred 1F0 LDB / ADD / SUB triads.
const picks = [
  {
    name: 'ldb_5060_1F0',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x1F0',
    body: '80 50 60 1F0',
    expected: pinOf(0x80, [0x50, 0x60, 0x1f0]),
  },
  {
    name: 'ldb_5160_1F0',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x1F0',
    body: '80 51 60 1F0',
    expected: pinOf(0x80, [0x51, 0x60, 0x1f0]),
  },
  {
    name: 'ldb_5260_1F0',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x1F0',
    body: '80 52 60 1F0',
    expected: pinOf(0x80, [0x52, 0x60, 0x1f0]),
  },
  {
    name: 'addimm_h50_1F0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x1F0',
    body: '62 50 1F0',
    expected: pinOf(0x62, [0x50, 0x1f0]),
  },
  {
    name: 'addimm_h51_1F0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x1F0',
    body: '62 51 1F0',
    expected: pinOf(0x62, [0x51, 0x1f0]),
  },
  {
    name: 'addimm_h52_1F0',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 52 1F0',
    expected: pinOf(0x62, [0x52, 0x1f0]),
  },
  {
    name: 'subimm_h50_1F0',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x1F0',
    body: '61 50 1F0',
    expected: pinOf(0x61, [0x50, 0x1f0]),
  },
  {
    name: 'subimm_h51_1F0',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x1F0',
    body: '61 51 1F0',
    expected: pinOf(0x61, [0x51, 0x1f0]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-85-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 653;
const SEL0 = 0x293;
const PIN = '63204ed031f1ad84c28688effab4ef4148b7c9e6277c1a08d68a7067dfe56aa1';
...
const log = `# parallel-batch-85 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-090 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-85-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
