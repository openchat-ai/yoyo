/**
 * parallel-batch-37 probe — scratch-only fresh picks after H_262..H_269.
 * Mirror parallel-batch-36-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_269. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// LDB oo≥0x80 → imm32 add (48 81 c0) → 26B.
// SUB-IMM/ADD-IMM imm=0x90 (>127) → imm32.
const picks = [
  {
    name: 'addimm_h51_90',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x90',
    body: '62 51 90',
    expected: pinOf(0x62, [0x51, 0x90]),
  },
  {
    name: 'addimm_h52_90',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x90',
    body: '62 52 90',
    expected: pinOf(0x62, [0x52, 0x90]),
  },
  {
    name: 'subimm_h50_90',
    opcode: '0x61 SUB-IMM',
  ...
  },
  {
    ...
    args: 'slot=0x52 imm=0x90',
    body: '61 52 90',
    expected: pinOf(0x61, [0x52, 0x90]),
  },
  {
    name: 'ldb_5060_b0',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0xB0',
    body: '80 50 60 B0',
    expected: pinOf(0x80, [0x50, 0x60, 0xb0]),
  },
  {
    name: 'ldb_5160_b0',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0xB0',
    body: '80 51 60 B0',
    expected: pinOf(0x80, [0x51, 0x60, 0xb0]),
  },
  {
    name: 'ldb_5260_b0',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0xB0',
    body: '80 52 60 B0',
    expected: pinOf(0x80, [0x52, 0x60, 0xb0]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-37-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 270 + i;
    const sel = (0x114 + i).toString(16).toUpperCase();
...
const PIN = 'afceb388015dd4a7e7a2de16a109eb8649189bb28471d021bb4b82eeaa9d1311';

const log = `# parallel-batch-37 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-042 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${270 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-37-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
