/**
 * parallel-batch-62 probe — scratch-only fresh picks after H_461..H_468.
 * Mirror parallel-batch-61-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_468. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 148 ADD triad + start 148 SUB triad + start 150 LDB triad + ADD-IMM 50 150.
const picks = [
  {
    name: 'addimm_h52_148',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x148',
    body: '62 52 148',
    expected: pinOf(0x62, [0x52, 0x148]),
  },
  {
    name: 'subimm_h50_148',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x148',
    body: '61 50 148',
    expected: pinOf(0x61, [0x50, 0x148]),
  },
  {
    name: 'subimm_h51_148',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x148',
    body: '61 51 148',
    expected: pinOf(0x61, [0x51, 0x148]),
  },
  {
    name: 'subimm_h52_148',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x148',
    body: '61 52 148',
    expected: pinOf(0x61, [0x52, 0x148]),
  },
  {
    name: 'ldb_5060_150',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x150',
    body: '80 50 60 150',
    expected: pinOf(0x80, [0x50, 0x60, 0x150]),
  },
  {
    name: 'ldb_5160_150',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x150',
    body: '80 51 60 150',
    expected: pinOf(0x80, [0x51, 0x60, 0x150]),
  },
  {
    name: 'ldb_5260_150',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x150',
    body: '80 52 60 150',
    expected: pinOf(0x80, [0x52, 0x60, 0x150]),
  },
  {
    name: 'addimm_h50_150',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x150',
    body: '62 50 150',
    expected: pinOf(0x62, [0x50, 0x150]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-62-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 469;
const SEL0 = 0x1db;
const PIN = 'deaf40134394a58d9e81fd3a8f55c4ec9110fc93ad8d366e547f0628144dd098';
...
const log = `# parallel-batch-62 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-067 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-62-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
