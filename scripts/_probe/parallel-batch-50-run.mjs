/**
 * parallel-batch-50 probe — scratch-only fresh picks after H_366..H_373.
 * Mirror parallel-batch-49-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_373. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// LDB/ADD/SUB imm=F8 triad — next oo/imm after F0.
const picks = [
  {
    name: 'ldb_5060_f8',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0xF8',
    body: '80 50 60 F8',
    expected: pinOf(0x80, [0x50, 0x60, 0xf8]),
  },
  {
    name: 'ldb_5160_f8',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0xF8',
    body: '80 51 60 F8',
    expected: pinOf(0x80, [0x51, 0x60, 0xf8]),
  },
  {
    name: 'ldb_5260_f8',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0xF8',
    body: '80 52 60 F8',
    expected: pinOf(0x80, [0x52, 0x60, 0xf8]),
  },
  {
    name: 'addimm_h50_f8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0xF8',
    body: '62 50 F8',
    expected: pinOf(0x62, [0x50, 0xf8]),
  },
  {
    name: 'addimm_h51_f8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0xF8',
    body: '62 51 F8',
    expected: pinOf(0x62, [0x51, 0xf8]),
  },
  {
    name: 'addimm_h52_f8',
    opcode: '0x62 ADD-IMM',
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-50-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 374;
const SEL0 = 0x17c;
const PIN = 'fba1f97e01a9ef7e6285451fe34b6b52a972caf99ae81f93518563d7eb1ec442';
...
const log = `# parallel-batch-50 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-055 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-50-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
