/**
 * parallel-batch-70 probe — scratch-only fresh picks after H_525..H_532.
 * Mirror parallel-batch-69-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_532. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish deferred 180 SUB triad + start 188 LDB/ADD triads.
const picks = [
  {
    name: 'subimm_h50_180',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x180',
    body: '61 50 180',
    expected: pinOf(0x61, [0x50, 0x180]),
  },
  {
    name: 'subimm_h51_180',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x180',
    body: '61 51 180',
    expected: pinOf(0x61, [0x51, 0x180]),
  },
  {
    name: 'subimm_h52_180',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x180',
    body: '61 52 180',
    expected: pinOf(0x61, [0x52, 0x180]),
  },
  {
    name: 'ldb_5060_188',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x188',
    body: '80 50 60 188',
    expected: pinOf(0x80, [0x50, 0x60, 0x188]),
  },
  {
    name: 'ldb_5160_188',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x188',
    body: '80 51 60 188',
    expected: pinOf(0x80, [0x51, 0x60, 0x188]),
  },
  {
    name: 'ldb_5260_188',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x188',
    body: '80 52 60 188',
    expected: pinOf(0x80, [0x52, 0x60, 0x188]),
  },
  {
    name: 'addimm_h50_188',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x188',
    body: '62 50 188',
    expected: pinOf(0x62, [0x50, 0x188]),
  },
  {
    name: 'addimm_h51_188',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x188',
    body: '62 51 188',
    expected: pinOf(0x62, [0x51, 0x188]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-70-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 533;
const SEL0 = 0x21b;
const PIN = '69f1bb2f223e28673dfb97de72b1305d451313a4865d02e766ed947748a10cf9';
...
const log = `# parallel-batch-70 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-075 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-70-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
