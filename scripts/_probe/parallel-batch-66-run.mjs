/**
 * parallel-batch-66 probe — scratch-only fresh picks after H_493..H_500.
 * Mirror parallel-batch-65-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_500. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 168 LDB triad + start 168 ADD/SUB triads.
const picks = [
  {
    name: 'ldb_5160_168',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x168',
    body: '80 51 60 168',
    expected: pinOf(0x80, [0x51, 0x60, 0x168]),
  },
  {
    name: 'ldb_5260_168',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x168',
    body: '80 52 60 168',
    expected: pinOf(0x80, [0x52, 0x60, 0x168]),
  },
  {
    name: 'addimm_h50_168',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x168',
    body: '62 50 168',
    expected: pinOf(0x62, [0x50, 0x168]),
  },
  {
    name: 'addimm_h51_168',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x168',
    body: '62 51 168',
    expected: pinOf(0x62, [0x51, 0x168]),
  },
  {
    name: 'addimm_h52_168',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x168',
    body: '62 52 168',
    expected: pinOf(0x62, [0x52, 0x168]),
  },
  {
    name: 'subimm_h50_168',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x168',
    body: '61 50 168',
    expected: pinOf(0x61, [0x50, 0x168]),
  },
  {
    name: 'subimm_h51_168',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x168',
    body: '61 51 168',
    expected: pinOf(0x61, [0x51, 0x168]),
  },
  {
    name: 'subimm_h52_168',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x168',
    body: '61 52 168',
    expected: pinOf(0x61, [0x52, 0x168]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-66-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 501;
const SEL0 = 0x1fb;
const PIN = '1f070530a91ca949696f7552fc5d3b3690f00630a191ce25662ee33951314e41';
...
const log = `# parallel-batch-66 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-071 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-66-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
