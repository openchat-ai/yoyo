/**
 * parallel-batch-41 probe — scratch-only fresh picks after H_294..H_301.
 * Mirror parallel-batch-40-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_301. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// LDB oo≥0x80 → imm32 add (48 81 c0) → 26B.
// SUB-IMM/ADD-IMM imm=0xB0 (>127) → imm32 → 22B.
const picks = [
  {
    name: 'ldb_5060_c8',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0xC8',
    body: '80 50 60 C8',
    expected: pinOf(0x80, [0x50, 0x60, 0xc8]),
  },
  {
    name: 'ldb_5160_c8',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0xC8',
    body: '80 51 60 C8',
    expected: pinOf(0x80, [0x51, 0x60, 0xc8]),
  },
  {
    name: 'ldb_5260_c8',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0xC8',
    body: '80 52 60 C8',
    expected: pinOf(0x80, [0x52, 0x60, 0xc8]),
  },
  {
    name: 'addimm_h50_b0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0xB0',
    body: '62 50 B0',
    expected: pinOf(0x62, [0x50, 0xb0]),
  },
  {
    name: 'addimm_h51_b0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0xB0',
    body: '62 51 B0',
    expected: pinOf(0x62, [0x51, 0xb0]),
  },
  {
    name: 'addimm_h52_b0',
    opcode: '0x62 ADD-IMM',
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-41-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 302 + i;
    const sel = (0x134 + i).toString(16).toUpperCase();
...
const PIN = '422c843275989ac30c1ba7406c7ff47076310df79ef0c3193903bca15460afde';

const log = `# parallel-batch-41 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-046 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${302 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-41-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
