/**
 * parallel-batch-61 probe — scratch-only fresh picks after H_453..H_460.
 * Mirror parallel-batch-60-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_460. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Start 140 SUB triad + start 148 LDB/ADD triads.
const picks = [
  {
    name: 'subimm_h50_140',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 50 140',
    expected: pinOf(0x61, [0x50, 0x140]),
  },
  {
    name: 'subimm_h51_140',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 51 140',
    expected: pinOf(0x61, [0x51, 0x140]),
  },
  {
    name: 'subimm_h52_140',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 52 140',
    expected: pinOf(0x61, [0x52, 0x140]),
  },
  {
    name: 'ldb_5060_148',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x148',
    body: '80 50 60 148',
    expected: pinOf(0x80, [0x50, 0x60, 0x148]),
  },
  {
    name: 'ldb_5160_148',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x148',
    body: '80 51 60 148',
    expected: pinOf(0x80, [0x51, 0x60, 0x148]),
  },
  {
    name: 'ldb_5260_148',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x148',
    body: '80 52 60 148',
    expected: pinOf(0x80, [0x52, 0x60, 0x148]),
  },
  {
    name: 'addimm_h50_148',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 50 148',
    expected: pinOf(0x62, [0x50, 0x148]),
  },
  {
    name: 'addimm_h51_148',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 51 148',
    expected: pinOf(0x62, [0x51, 0x148]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-61-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 461;
const SEL0 = 0x1d3;
const PIN = 'd52ed6373d5b085118d5a601ac8f25b8a529e7c16b36b6dd3bce2115d73ec080';
...
const log = `# parallel-batch-61 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-066 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-61-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
