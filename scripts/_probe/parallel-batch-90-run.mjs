/**
 * parallel-batch-90 probe — scratch-only fresh picks after H_685..H_692.
 * Mirror parallel-batch-89-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_692. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish deferred 210 ADD triad; start 210 SUB triad; start 218 LDB triad.
const picks = [
  {
    name: 'addimm_h51_210',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x210',
    body: '62 51 210',
    expected: pinOf(0x62, [0x51, 0x210]),
  },
  {
    name: 'addimm_h52_210',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x210',
    body: '62 52 210',
    expected: pinOf(0x62, [0x52, 0x210]),
  },
  {
    name: 'subimm_h50_210',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 50 210',
    expected: pinOf(0x61, [0x50, 0x210]),
  },
  {
    name: 'subimm_h51_210',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x210',
    body: '61 51 210',
    expected: pinOf(0x61, [0x51, 0x210]),
  },
  {
    name: 'subimm_h52_210',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x210',
    body: '61 52 210',
    expected: pinOf(0x61, [0x52, 0x210]),
  },
  {
    name: 'ldb_5060_218',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x218',
    body: '80 50 60 218',
    expected: pinOf(0x80, [0x50, 0x60, 0x218]),
  },
  {
    name: 'ldb_5160_218',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x218',
    body: '80 51 60 218',
    expected: pinOf(0x80, [0x51, 0x60, 0x218]),
  },
  {
    name: 'ldb_5260_218',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x218',
    body: '80 52 60 218',
    expected: pinOf(0x80, [0x52, 0x60, 0x218]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-90-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 693;
const SEL0 = 0x2bb;
const PIN = 'aef6d89f98ceb7c8d9770950da9a584d7165f7e0d6713fc30c1d3f14c92552ee';
...
const log = `# parallel-batch-90 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-095 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-90-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
