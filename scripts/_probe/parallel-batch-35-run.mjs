/**
 * parallel-batch-35 probe — scratch-only fresh picks after H_246..H_253.
 * Mirror parallel-batch-34-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_253. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// LDB oo≥0x80 → imm32 add (48 81 c0) → 26B.
// SUB-IMM/ADD-IMM imm=0x80/0x88 (>127) → imm32.
const picks = [
  {
    name: 'ldb_5060_a0',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0xA0',
    body: '80 50 60 A0',
    expected: pinOf(0x80, [0x50, 0x60, 0xa0]),
  },
  {
    name: 'ldb_5160_a0',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0xA0',
    body: '80 51 60 A0',
    expected: pinOf(0x80, [0x51, 0x60, 0xa0]),
  },
  {
    name: 'ldb_5260_a0',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0xA0',
    body: '80 52 60 A0',
    expected: pinOf(0x80, [0x52, 0x60, 0xa0]),
  },
  {
    name: 'subimm_h50_80',
    opcode: '0x61 SUB-IMM',
  ...
  },
  {
    ...
    body: '61 52 80',
    expected: pinOf(0x61, [0x52, 0x80]),
  },
  {
    name: 'addimm_h50_88',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x88',
    body: '62 50 88',
    expected: pinOf(0x62, [0x50, 0x88]),
  },
  {
    name: 'addimm_h51_88',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x88',
    body: '62 51 88',
    expected: pinOf(0x62, [0x51, 0x88]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-35-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 254 + i;
    const sel = (0x104 + i).toString(16).toUpperCase();
...
const PIN = 'a58ead289233c42ba1c6e9a84aedb6218176aad27ecd5cbdd0d4659a2e5bc187';

const log = `# parallel-batch-35 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-040 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${254 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-35-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
