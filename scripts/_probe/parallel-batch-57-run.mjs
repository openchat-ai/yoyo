/**
 * parallel-batch-57 probe — scratch-only fresh picks after H_422..H_429.
 * Mirror parallel-batch-56-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_429. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 128 LDB triad (dd=52); start 128 ADD/SUB triads.
const picks = [
  {
    name: 'ldb_5260_128',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x128',
    body: '80 52 60 128',
    expected: pinOf(0x80, [0x52, 0x60, 0x128]),
  },
  {
    name: 'addimm_h50_128',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x128',
    body: '62 50 128',
    expected: pinOf(0x62, [0x50, 0x128]),
  },
  {
    name: 'addimm_h51_128',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x128',
    body: '62 51 128',
    expected: pinOf(0x62, [0x51, 0x128]),
  },
  {
    name: 'addimm_h52_128',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x128',
    body: '62 52 128',
    expected: pinOf(0x62, [0x52, 0x128]),
  },
  {
    name: 'subimm_h50_128',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x128',
    body: '61 50 128',
    expected: pinOf(0x61, [0x50, 0x128]),
  },
  {
    name: 'subimm_h51_128',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x128',
    body: '61 51 128',
    expected: pinOf(0x61, [0x51, 0x128]),
  },
  {
    name: 'subimm_h52_128',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x128',
    body: '61 52 128',
    expected: pinOf(0x61, [0x52, 0x128]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-57-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 430;
const SEL0 = 0x1b4;
const PIN = 'c5b95f3792afa572a774aa41d22dd49fb27b6905aa7ab891273b77db49a3af0a';
...
const log = `# parallel-batch-57 Log · 7-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-062 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-57-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
