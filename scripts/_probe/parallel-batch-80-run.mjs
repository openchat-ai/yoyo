/**
 * parallel-batch-80 probe — scratch-only fresh picks after H_605..H_612.
 * Mirror parallel-batch-79-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_612. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 1C8 ADD; start 1C8 SUB; start 1D0 LDB; start 1D0 ADD.
const picks = [
  {
    name: 'addimm_h52_1C8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x1C8',
    body: '62 52 1C8',
    expected: pinOf(0x62, [0x52, 0x1c8]),
  },
  {
    name: 'subimm_h50_1C8',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 50 1C8',
    expected: pinOf(0x61, [0x50, 0x1c8]),
  },
  {
    name: 'subimm_h51_1C8',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 51 1C8',
    expected: pinOf(0x61, [0x51, 0x1c8]),
  },
  {
    name: 'subimm_h52_1C8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x1C8',
    body: '61 52 1C8',
    expected: pinOf(0x61, [0x52, 0x1c8]),
  },
  {
    name: 'ldb_5060_1D0',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x1D0',
    body: '80 50 60 1D0',
    expected: pinOf(0x80, [0x50, 0x60, 0x1d0]),
  },
  {
    name: 'ldb_5160_1D0',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x1D0',
    body: '80 51 60 1D0',
    expected: pinOf(0x80, [0x51, 0x60, 0x1d0]),
  },
  {
    name: 'ldb_5260_1D0',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x1D0',
    body: '80 52 60 1D0',
    expected: pinOf(0x80, [0x52, 0x60, 0x1d0]),
  },
  {
    name: 'addimm_h50_1D0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x1D0',
    body: '62 50 1D0',
    expected: pinOf(0x62, [0x50, 0x1d0]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-80-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 613;
const SEL0 = 0x26b;
const PIN = '58b9ca6ef16f3ee48e22fae95f20dd6f6fa3492705659dfe181ec7857e9cf231';
...
const log = `# parallel-batch-80 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-085 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-80-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
