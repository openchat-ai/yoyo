/**
 * parallel-batch-87 probe — scratch-only fresh picks after H_661..H_668.
 * Mirror parallel-batch-86-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_668. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish deferred 1F8 SUB triad; start 200 LDB/ADD ladder.
const picks = [
  {
    name: 'subimm_h51_1F8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x1F8',
    body: '61 51 1F8',
    expected: pinOf(0x61, [0x51, 0x1f8]),
  },
  {
    name: 'subimm_h52_1F8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x1F8',
    body: '61 52 1F8',
    expected: pinOf(0x61, [0x52, 0x1f8]),
  },
  {
    name: 'ldb_5060_200',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x200',
    body: '80 50 60 200',
    expected: pinOf(0x80, [0x50, 0x60, 0x200]),
  },
  {
    name: 'ldb_5160_200',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x200',
    body: '80 51 60 200',
    expected: pinOf(0x80, [0x51, 0x60, 0x200]),
  },
  {
    name: 'ldb_5260_200',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x200',
    body: '80 52 60 200',
    expected: pinOf(0x80, [0x52, 0x60, 0x200]),
  },
  {
    name: 'addimm_h50_200',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x200',
    body: '62 50 200',
    expected: pinOf(0x62, [0x50, 0x200]),
  },
  {
    name: 'addimm_h51_200',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x200',
    body: '62 51 200',
    expected: pinOf(0x62, [0x51, 0x200]),
  },
  {
    name: 'addimm_h52_200',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x200',
    body: '62 52 200',
    expected: pinOf(0x62, [0x52, 0x200]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-87-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 669;
const SEL0 = 0x2a3;
const PIN = '1991af8484d67ec19980bf14771d523d332f85c9974e1da09d45496baf46ebb5';
...
const log = `# parallel-batch-87 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-092 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-87-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
