/**
 * parallel-batch-25 probe — scratch-only fresh picks after H_166..H_173.
 * Mirror parallel-batch-24-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_173. Skip early-dup INC/DEC 50, GET/ORV/ADDV/SUBV 50 51,
// IMUL 50 51. Skip LDB 50 60 60 (H_46). Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
const picks = [
  {
    name: 'set_50_deadc0de',
    opcode: '0x30 SET',
    args: 'slot=0x50 imm=0xDEADC0DE',
    body: '30 50 DEADC0DE',
    expected: pinOf(0x30, [0x50, 0xdeadc0de]),
  },
  {
    name: 'ldb_5160_60',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x60',
    body: '80 51 60 60',
    expected: pinOf(0x80, [0x51, 0x60, 0x60]),
  },
  {
    name: 'ldb_5260_60',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x60',
    body: '80 52 60 60',
    expected: pinOf(0x80, [0x52, 0x60, 0x60]),
  },
  {
    name: 'addimm_h50_40',
    opcode: '0x62 ADD-IMM',
  ...
  },
  {
    ...
    body: '61 52 3C',
    expected: pinOf(0x61, [0x52, 0x3c]),
  },
  {
    name: 'set_51_deadc0de',
    opcode: '0x30 SET',
    args: 'slot=0x51 imm=0xDEADC0DE',
    body: '30 51 DEADC0DE',
    expected: pinOf(0x30, [0x51, 0xdeadc0de]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-25-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 174 + i;
    const sel = (0xb4 + i).toString(16).toUpperCase().padStart(2, '0');
...
const PIN = '9fddb56b31ab513c92e4435193619de1193f4ea543bbb4b2a239531eeefae0ea';

const log = `# parallel-batch-25 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-030 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${174 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-25-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
