/**
 * parallel-batch-32 probe — scratch-only fresh picks after H_222..H_229.
 * Mirror parallel-batch-31-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_229. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
...
const picks = [
  ...
  },
  {
    ...
    body: '61 52 68',
    expected: pinOf(0x61, [0x52, 0x68]),
  },
  {
    name: 'ldb_5060_90',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x90',
    body: '80 50 60 90',
    expected: pinOf(0x80, [0x50, 0x60, 0x90]),
  },
  {
    name: 'ldb_5160_90',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x90',
    body: '80 51 60 90',
    expected: pinOf(0x80, [0x51, 0x60, 0x90]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-32-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 230 + i;
    const sel = (0xec + i).toString(16).toUpperCase().padStart(2, '0');
...
const PIN = '39d454a194359d1d682b0638381fa14cbdec617e707f26b2b2405e05be7f9ede';

const log = `# parallel-batch-32 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-037 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${230 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-32-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
