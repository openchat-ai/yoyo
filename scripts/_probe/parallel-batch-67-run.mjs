/**
 * parallel-batch-67 probe — scratch-only fresh picks after H_501..H_508.
 * Mirror parallel-batch-66-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_508. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Start 170 LDB triad + start 170 ADD triad + start 170 SUB (50/51).
const picks = [
  {
    name: 'ldb_5060_170',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x170',
    body: '80 50 60 170',
    expected: pinOf(0x80, [0x50, 0x60, 0x170]),
  },
  {
    name: 'ldb_5160_170',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x170',
    body: '80 51 60 170',
    expected: pinOf(0x80, [0x51, 0x60, 0x170]),
  },
  {
    name: 'ldb_5260_170',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x170',
    body: '80 52 60 170',
    expected: pinOf(0x80, [0x52, 0x60, 0x170]),
  },
  {
    name: 'addimm_h50_170',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x170',
    body: '62 50 170',
    expected: pinOf(0x62, [0x50, 0x170]),
  },
  {
    name: 'addimm_h51_170',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x170',
    body: '62 51 170',
    expected: pinOf(0x62, [0x51, 0x170]),
  },
  {
    name: 'addimm_h52_170',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 52 170',
    expected: pinOf(0x62, [0x52, 0x170]),
  },
  {
    name: 'subimm_h50_170',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x170',
    body: '61 50 170',
    expected: pinOf(0x61, [0x50, 0x170]),
  },
  {
    name: 'subimm_h51_170',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x170',
    body: '61 51 170',
    expected: pinOf(0x61, [0x51, 0x170]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-67-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 509;
const SEL0 = 0x203;
const PIN = 'e1554db8dcce9946348a88383bed73939d4a835e8dc0989a2788a72a590e6a6b';
...
const log = `# parallel-batch-67 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-072 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-67-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
