/**
 * parallel-batch-56 probe — scratch-only fresh picks after H_414..H_421.
 * Mirror parallel-batch-55-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_421. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Start 120 ADD triad; 120 SUB triad; LDB oo=128 (dd=50/51).
const picks = [
  {
    name: 'addimm_h50_120',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x120',
    body: '62 50 120',
    expected: pinOf(0x62, [0x50, 0x120]),
  },
  {
    name: 'addimm_h51_120',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x120',
    body: '62 51 120',
    expected: pinOf(0x62, [0x51, 0x120]),
  },
  {
    name: 'addimm_h52_120',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x120',
    body: '62 52 120',
    expected: pinOf(0x62, [0x52, 0x120]),
  },
  {
    name: 'subimm_h50_120',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x120',
    body: '61 50 120',
    expected: pinOf(0x61, [0x50, 0x120]),
  },
  {
    name: 'subimm_h51_120',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x120',
    body: '61 51 120',
    expected: pinOf(0x61, [0x51, 0x120]),
  },
  {
    name: 'subimm_h52_120',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x120',
    body: '61 52 120',
    expected: pinOf(0x61, [0x52, 0x120]),
  },
  {
    name: 'ldb_5060_128',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x128',
    body: '80 50 60 128',
    expected: pinOf(0x80, [0x50, 0x60, 0x128]),
  },
  {
    name: 'ldb_5160_128',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x128',
    body: '80 51 60 128',
    expected: pinOf(0x80, [0x51, 0x60, 0x128]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-56-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 422;
const SEL0 = 0x1ac;
const PIN = 'd4437da8f517c8d37c1335b590cae185c0be035d120d84f5ffa0e9354ae484a9';
...
const log = `# parallel-batch-56 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-061 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-56-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
