/**
 * parallel-batch-69 probe — scratch-only fresh picks after H_517..H_524.
 * Mirror parallel-batch-68-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_524. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 178 SUB triad + start 180 LDB/ADD triads.
const picks = [
  {
    name: 'subimm_h51_178',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x178',
    body: '61 51 178',
    expected: pinOf(0x61, [0x51, 0x178]),
  },
  {
    name: 'subimm_h52_178',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x178',
    body: '61 52 178',
    expected: pinOf(0x61, [0x52, 0x178]),
  },
  {
    name: 'ldb_5060_180',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x180',
    body: '80 50 60 180',
    expected: pinOf(0x80, [0x50, 0x60, 0x180]),
  },
  {
    name: 'ldb_5160_180',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x180',
    body: '80 51 60 180',
    expected: pinOf(0x80, [0x51, 0x60, 0x180]),
  },
  {
    name: 'ldb_5260_180',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x180',
    body: '80 52 60 180',
    expected: pinOf(0x80, [0x52, 0x60, 0x180]),
  },
  {
    name: 'addimm_h50_180',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 50 180',
    expected: pinOf(0x62, [0x50, 0x180]),
  },
  {
    name: 'addimm_h51_180',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 51 180',
    expected: pinOf(0x62, [0x51, 0x180]),
  },
  {
    name: 'addimm_h52_180',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 52 180',
    expected: pinOf(0x62, [0x52, 0x180]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-69-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 525;
const SEL0 = 0x213;
const PIN = '9243965c886555e99575615e4637331b6c2a49573d50ec183fb616c3ae3d2d98';
...
const log = `# parallel-batch-69 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-074 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-69-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
