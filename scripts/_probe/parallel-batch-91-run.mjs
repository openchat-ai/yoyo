/**
 * parallel-batch-91 probe — scratch-only fresh picks after H_693..H_700.
 * Mirror parallel-batch-90-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_700. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Start deferred 218 ADD + SUB triads; start 220 LDB ladder (2 of 3).
const picks = [
  {
    name: 'addimm_h50_218',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x218',
    body: '62 50 218',
    expected: pinOf(0x62, [0x50, 0x218]),
  },
  {
    name: 'addimm_h51_218',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x218',
    body: '62 51 218',
    expected: pinOf(0x62, [0x51, 0x218]),
  },
  {
    name: 'addimm_h52_218',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x218',
    body: '62 52 218',
    expected: pinOf(0x62, [0x52, 0x218]),
  },
  {
    name: 'subimm_h50_218',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x218',
    body: '61 50 218',
    expected: pinOf(0x61, [0x50, 0x218]),
  },
  {
    name: 'subimm_h51_218',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x218',
    body: '61 51 218',
    expected: pinOf(0x61, [0x51, 0x218]),
  },
  {
    name: 'subimm_h52_218',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x218',
    body: '61 52 218',
    expected: pinOf(0x61, [0x52, 0x218]),
  },
  {
    name: 'ldb_5060_220',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x220',
    body: '80 50 60 220',
    expected: pinOf(0x80, [0x50, 0x60, 0x220]),
  },
  {
    name: 'ldb_5160_220',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x220',
    body: '80 51 60 220',
    expected: pinOf(0x80, [0x51, 0x60, 0x220]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-91-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 701;
const SEL0 = 0x2c3;
const PIN = '0a02f49ed0c94a2df5078022a7737c92d4021cab62c41dcbbfc5bb728f32f29c';
...
const log = `# parallel-batch-91 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-096 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-91-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
