/**
 * parallel-batch-52 probe — scratch-only fresh picks after H_382..H_389.
 * Mirror parallel-batch-51-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_389. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 100 SUB triad; then LDB/ADD-IMM imm=108 triad.
const picks = [
  {
    name: 'subimm_h51_100',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x100',
    body: '61 51 100',
    expected: pinOf(0x61, [0x51, 0x100]),
  },
  {
    name: 'subimm_h52_100',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x100',
    body: '61 52 100',
    expected: pinOf(0x61, [0x52, 0x100]),
  },
  {
    name: 'ldb_5060_108',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x108',
    body: '80 50 60 108',
    expected: pinOf(0x80, [0x50, 0x60, 0x108]),
  },
  {
    name: 'ldb_5160_108',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x108',
    body: '80 51 60 108',
    expected: pinOf(0x80, [0x51, 0x60, 0x108]),
  },
  {
    name: 'ldb_5260_108',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x108',
    body: '80 52 60 108',
    expected: pinOf(0x80, [0x52, 0x60, 0x108]),
  },
  {
    name: 'addimm_h50_108',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 50 108',
    expected: pinOf(0x62, [0x50, 0x108]),
  },
  {
    name: 'addimm_h51_108',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 51 108',
    expected: pinOf(0x62, [0x51, 0x108]),
  },
  {
    name: 'addimm_h52_108',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 52 108',
    expected: pinOf(0x62, [0x52, 0x108]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-52-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 390;
const SEL0 = 0x18c;
const PIN = '0643c8f550fbb85d6e85eac409cf7ac90a26d7fece1b33bcfe04af260a9f2d5a';
...
const log = `# parallel-batch-52 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-057 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-52-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
