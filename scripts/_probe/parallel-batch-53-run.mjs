/**
 * parallel-batch-53 probe — scratch-only fresh picks after H_390..H_397.
 * Mirror parallel-batch-52-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_397. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 108 SUB triad; LDB oo=110 triad; ADD-IMM imm=110 pair.
const picks = [
  {
    name: 'subimm_h50_108',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x108',
    body: '61 50 108',
    expected: pinOf(0x61, [0x50, 0x108]),
  },
  {
    name: 'subimm_h51_108',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x108',
    body: '61 51 108',
    expected: pinOf(0x61, [0x51, 0x108]),
  },
  {
    name: 'subimm_h52_108',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x108',
    body: '61 52 108',
    expected: pinOf(0x61, [0x52, 0x108]),
  },
  {
    name: 'ldb_5060_110',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x110',
    body: '80 50 60 110',
    expected: pinOf(0x80, [0x50, 0x60, 0x110]),
  },
  {
    name: 'ldb_5160_110',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x110',
    body: '80 51 60 110',
    expected: pinOf(0x80, [0x51, 0x60, 0x110]),
  },
  {
    name: 'ldb_5260_110',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x110',
    body: '80 52 60 110',
    expected: pinOf(0x80, [0x52, 0x60, 0x110]),
  },
  {
    name: 'addimm_h50_110',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x110',
    body: '62 50 110',
    expected: pinOf(0x62, [0x50, 0x110]),
  },
  {
    name: 'addimm_h51_110',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x110',
    body: '62 51 110',
    expected: pinOf(0x62, [0x51, 0x110]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-53-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 398;
const SEL0 = 0x194;
const PIN = 'c258ff3271396e1822dba5baf34c98aae7003f19c10a916a0aa3967142f5c2dc';
...
const log = `# parallel-batch-53 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-058 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-53-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
