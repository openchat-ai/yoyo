/**
 * parallel-batch-86 probe — scratch-only fresh picks after H_653..H_660.
 * Mirror parallel-batch-85-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_660. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish deferred 1F0 SUB triad; start 1F8 LDB/ADD/SUB ladder.
const picks = [
  {
    name: 'subimm_h52_1F0',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x1F0',
    body: '61 52 1F0',
    expected: pinOf(0x61, [0x52, 0x1f0]),
  },
  {
    name: 'ldb_5060_1F8',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x1F8',
    body: '80 50 60 1F8',
    expected: pinOf(0x80, [0x50, 0x60, 0x1f8]),
  },
  {
    name: 'ldb_5160_1F8',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x1F8',
    body: '80 51 60 1F8',
    expected: pinOf(0x80, [0x51, 0x60, 0x1f8]),
  },
  {
    name: 'ldb_5260_1F8',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x1F8',
    body: '80 52 60 1F8',
    expected: pinOf(0x80, [0x52, 0x60, 0x1f8]),
  },
  {
    name: 'addimm_h50_1F8',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x1F8',
    body: '62 50 1F8',
    expected: pinOf(0x62, [0x50, 0x1f8]),
  },
  {
    name: 'addimm_h51_1F8',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 51 1F8',
    expected: pinOf(0x62, [0x51, 0x1f8]),
  },
  {
    name: 'addimm_h52_1F8',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 52 1F8',
    expected: pinOf(0x62, [0x52, 0x1f8]),
  },
  {
    name: 'subimm_h50_1F8',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x1F8',
    body: '61 50 1F8',
    expected: pinOf(0x61, [0x50, 0x1f8]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-86-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 661;
const SEL0 = 0x29b;
const PIN = '339bd482ae784eb8a80f7176ef5d7c6f3c90b0e491b08c6103512860ab5b918a';
...
const log = `# parallel-batch-86 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-091 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-86-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
