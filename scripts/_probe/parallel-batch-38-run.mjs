/**
 * parallel-batch-38 probe — scratch-only fresh picks after H_270..H_277.
 * Mirror parallel-batch-37-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_277. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// LDB oo≥0x80 → imm32 add (48 81 c0) → 26B.
// SUB-IMM/ADD-IMM imm=0x98 (>127) → imm32 → 22B.
const picks = [
  {
    name: 'addimm_h50_98',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x98',
    body: '62 50 98',
    expected: pinOf(0x62, [0x50, 0x98]),
  },
  {
    name: 'addimm_h51_98',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x98',
    body: '62 51 98',
    expected: pinOf(0x62, [0x51, 0x98]),
  },
  {
    name: 'addimm_h52_98',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x98',
    body: '62 52 98',
    expected: pinOf(0x62, [0x52, 0x98]),
  },
  {
    name: 'subimm_h50_98',
    opcode: '0x61 SUB-IMM',
  ...
  },
  {
    ...
    args: 'slot=0x52 imm=0x98',
    body: '61 52 98',
    expected: pinOf(0x61, [0x52, 0x98]),
  },
  {
    name: 'ldb_5060_b8',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0xB8',
    body: '80 50 60 B8',
    expected: pinOf(0x80, [0x50, 0x60, 0xb8]),
  },
  {
    name: 'ldb_5160_b8',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0xB8',
    body: '80 51 60 B8',
    expected: pinOf(0x80, [0x51, 0x60, 0xb8]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-38-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 278 + i;
    const sel = (0x11c + i).toString(16).toUpperCase();
...
const PIN = '113decd0cbfa7a1106ae3f17f82ba7b6a135c8ad6a3b579b7c30978ffb96d7a0';

const log = `# parallel-batch-38 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-043 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${278 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-38-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
