/**
 * parallel-batch-93 probe — scratch-only fresh picks after H_709..H_716.
 * Mirror parallel-batch-92-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_716. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish deferred 228 LDB triad; start 228 ADD + SUB triads.
const picks = [
  {
    name: 'ldb_5160_228',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x228',
    body: '80 51 60 228',
    expected: pinOf(0x80, [0x51, 0x60, 0x228]),
  },
  {
    name: 'ldb_5260_228',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x228',
    body: '80 52 60 228',
    expected: pinOf(0x80, [0x52, 0x60, 0x228]),
  },
  {
    name: 'addimm_h50_228',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x228',
    body: '62 50 228',
    expected: pinOf(0x62, [0x50, 0x228]),
  },
  {
    name: 'addimm_h51_228',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x228',
    body: '62 51 228',
    expected: pinOf(0x62, [0x51, 0x228]),
  },
  {
    name: 'addimm_h52_228',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x228',
    body: '62 52 228',
    expected: pinOf(0x62, [0x52, 0x228]),
  },
  {
    name: 'subimm_h50_228',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x228',
    body: '61 50 228',
    expected: pinOf(0x61, [0x50, 0x228]),
  },
  {
    name: 'subimm_h51_228',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x228',
    body: '61 51 228',
    expected: pinOf(0x61, [0x51, 0x228]),
  },
  {
    name: 'subimm_h52_228',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x228',
    body: '61 52 228',
    expected: pinOf(0x61, [0x52, 0x228]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-93-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 717;
const SEL0 = 0x2d3;
const PIN = '8d4277255b098dc108295590e42155afd50ffca67fbab34ea1430ef615405136';
...
const log = `# parallel-batch-93 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-098 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-93-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
