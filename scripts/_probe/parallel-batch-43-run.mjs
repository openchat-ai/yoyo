/**
 * parallel-batch-43 probe — scratch-only fresh picks after H_310..H_317.
 * Mirror parallel-batch-42-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_317. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// LDB oo≥0x80 → imm32 add (48 81 c0) → 26B.
// SUB-IMM/ADD-IMM imm=0xC0 (>127) → imm32 → 22B.
const picks = [
  {
    name: 'ldb_5160_d0',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0xD0',
    body: '80 51 60 D0',
    expected: pinOf(0x80, [0x51, 0x60, 0xd0]),
  },
  {
    name: 'ldb_5260_d0',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0xD0',
    body: '80 52 60 D0',
    expected: pinOf(0x80, [0x52, 0x60, 0xd0]),
  },
  {
    name: 'addimm_h50_c0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0xC0',
    body: '62 50 C0',
    expected: pinOf(0x62, [0x50, 0xc0]),
  },
  {
    name: 'addimm_h51_c0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0xC0',
    body: '62 51 C0',
    expected: pinOf(0x62, [0x51, 0xc0]),
  },
  {
    name: 'addimm_h52_c0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0xC0',
    body: '62 52 C0',
    expected: pinOf(0x62, [0x52, 0xc0]),
  },
  {
    name: 'subimm_h50_c0',
    opcode: '0x61 SUB-IMM',
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-43-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 318 + i;
    const sel = (0x144 + i).toString(16).toUpperCase();
...
const PIN = '9c2f924a2780d64647f590c707d39330fa4bff0e69a2c243c0550956ec2d41a2';

const log = `# parallel-batch-43 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-048 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${318 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-43-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
