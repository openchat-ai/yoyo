/**
 * parallel-batch-63 probe — scratch-only fresh picks after H_469..H_476.
 * Mirror parallel-batch-62-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_476. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 150 ADD triad + start 150 SUB triad + start 158 LDB triad.
const picks = [
  {
    name: 'addimm_h51_150',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x150',
    body: '62 51 150',
    expected: pinOf(0x62, [0x51, 0x150]),
  },
  {
    name: 'addimm_h52_150',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x150',
    body: '62 52 150',
    expected: pinOf(0x62, [0x52, 0x150]),
  },
  {
    name: 'subimm_h50_150',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 50 150',
    expected: pinOf(0x61, [0x50, 0x150]),
  },
  {
    name: 'subimm_h51_150',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x150',
    body: '61 51 150',
    expected: pinOf(0x61, [0x51, 0x150]),
  },
  {
    name: 'subimm_h52_150',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x150',
    body: '61 52 150',
    expected: pinOf(0x61, [0x52, 0x150]),
  },
  {
    name: 'ldb_5060_158',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x158',
    body: '80 50 60 158',
    expected: pinOf(0x80, [0x50, 0x60, 0x158]),
  },
  {
    name: 'ldb_5160_158',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x158',
    body: '80 51 60 158',
    expected: pinOf(0x80, [0x51, 0x60, 0x158]),
  },
  {
    name: 'ldb_5260_158',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x158',
    body: '80 52 60 158',
    expected: pinOf(0x80, [0x52, 0x60, 0x158]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-63-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 477;
const SEL0 = 0x1e3;
const PIN = '2f81b43ba9e34a3bbc786fc9d308d0cc6d38c866dfdfd8e52a51bfed15acb5b8';
...
const log = `# parallel-batch-63 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-068 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-63-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
