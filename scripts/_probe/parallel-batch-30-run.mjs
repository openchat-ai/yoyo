/**
 * parallel-batch-30 probe — scratch-only fresh picks after H_206..H_213.
 * Mirror parallel-batch-29-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_213. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
...
const picks = [
  ...
  },
  {
    ...
    body: '61 51 58',
    expected: pinOf(0x61, [0x51, 0x58]),
  },
  {
    name: 'ldb_5160_80',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x80',
    body: '80 51 60 80',
    expected: pinOf(0x80, [0x51, 0x60, 0x80]),
  },
  {
    name: 'ldb_5260_80',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x80',
    body: '80 52 60 80',
    expected: pinOf(0x80, [0x52, 0x60, 0x80]),
  },
  {
    name: 'subimm_h52_58',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 52 58',
    expected: pinOf(0x61, [0x52, 0x58]),
  },
  {
    name: 'addimm_h50_68',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x68',
    body: '62 50 68',
    expected: pinOf(0x62, [0x50, 0x68]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-30-log.md');
const table = rows
  .map(
    (r, i) =>
...
const passTable = pass
  .map((r, i) => {
    const h = 214 + i;
    const sel = (0xdc + i).toString(16).toUpperCase().padStart(2, '0');
...
const PIN = '23f42236c6097a13e83a15c861d51845fbe1da64eadfabdb95fdeaca3ebe55f5';

const log = `# parallel-batch-30 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-035 Relock: \`${PIN}\`.
...
${pass.map((r, i) => `  - H_${214 + i} ${r.opcode} (${r.body}) — pin \`${r.pin}\``).join('\n')}
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-30-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
