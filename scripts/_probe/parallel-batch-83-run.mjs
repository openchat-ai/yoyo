/**
 * parallel-batch-83 probe — scratch-only fresh picks after H_629..H_636.
 * Mirror parallel-batch-82-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_636. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 1E0 LDB; start 1E0 ADD; start 1E0 SUB; start 1E8 LDB.
const picks = [
  {
    name: 'ldb_5260_1E0',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x1E0',
    body: '80 52 60 1E0',
    expected: pinOf(0x80, [0x52, 0x60, 0x1e0]),
  },
  {
    name: 'addimm_h50_1E0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x50 imm=0x1E0',
    body: '62 50 1E0',
    expected: pinOf(0x62, [0x50, 0x1e0]),
  },
  {
    name: 'addimm_h51_1E0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x51 imm=0x1E0',
    body: '62 51 1E0',
    expected: pinOf(0x62, [0x51, 0x1e0]),
  },
  {
    name: 'addimm_h52_1E0',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x1E0',
    body: '62 52 1E0',
    expected: pinOf(0x62, [0x52, 0x1e0]),
  },
  {
    name: 'subimm_h50_1E0',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x50 imm=0x1E0',
    body: '61 50 1E0',
    expected: pinOf(0x61, [0x50, 0x1e0]),
  },
  {
    name: 'subimm_h51_1E0',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x51 imm=0x1E0',
    body: '61 51 1E0',
    expected: pinOf(0x61, [0x51, 0x1e0]),
  },
  {
    name: 'subimm_h52_1E0',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x1E0',
    body: '61 52 1E0',
    expected: pinOf(0x61, [0x52, 0x1e0]),
  },
  {
    name: 'ldb_5060_1E8',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x1E8',
    body: '80 50 60 1E8',
    expected: pinOf(0x80, [0x50, 0x60, 0x1e8]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-83-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 637;
const SEL0 = 0x283;
const PIN = '697ad7847ba15e825ee7a2663be37eb71de542256a38f42ed2e7dc16ddca549c';
...
const log = `# parallel-batch-83 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-088 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-83-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
