/**
 * parallel-batch-54 probe — scratch-only fresh picks after H_398..H_405.
 * Mirror parallel-batch-53-run.mjs. Do NOT touch yoyo.ty / lock / goldens.
 */
...
// Fresh vs H_48..H_405. Skip D-1/D-2/D-3 / MEMCPY. No AND/XOR.
// Slot disp: 50→0x280, 51→0x288, 52→0x290, 60→0x300.
// Store disp LE: 50→80, 51→88, 52→90.
// Finish 110 ADD triad; 110 SUB triad; LDB oo=118 triad; ADD-IMM 50 118.
const picks = [
  {
    name: 'addimm_h52_110',
    opcode: '0x62 ADD-IMM',
    args: 'slot=0x52 imm=0x110',
    body: '62 52 110',
    expected: pinOf(0x62, [0x52, 0x110]),
  },
  {
    name: 'subimm_h50_110',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 50 110',
    expected: pinOf(0x61, [0x50, 0x110]),
  },
  {
    name: 'subimm_h51_110',
    opcode: '0x61 SUB-IMM',
    ...
    body: '61 51 110',
    expected: pinOf(0x61, [0x51, 0x110]),
  },
  {
    name: 'subimm_h52_110',
    opcode: '0x61 SUB-IMM',
    args: 'slot=0x52 imm=0x110',
    body: '61 52 110',
    expected: pinOf(0x61, [0x52, 0x110]),
  },
  {
    name: 'ldb_5060_118',
    opcode: '0x80 LDB',
    args: 'dd=0x50 ss=0x60 oo=0x118',
    body: '80 50 60 118',
    expected: pinOf(0x80, [0x50, 0x60, 0x118]),
  },
  {
    name: 'ldb_5160_118',
    opcode: '0x80 LDB',
    args: 'dd=0x51 ss=0x60 oo=0x118',
    body: '80 51 60 118',
    expected: pinOf(0x80, [0x51, 0x60, 0x118]),
  },
  {
    name: 'ldb_5260_118',
    opcode: '0x80 LDB',
    args: 'dd=0x52 ss=0x60 oo=0x118',
    body: '80 52 60 118',
    expected: pinOf(0x80, [0x52, 0x60, 0x118]),
  },
  {
    name: 'addimm_h50_118',
    opcode: '0x62 ADD-IMM',
    ...
    body: '62 50 118',
    expected: pinOf(0x62, [0x50, 0x118]),
  },
];
...
const logPath = path.join(ROOT, 'docs/auxdocs/parallel-batch-54-log.md');
const table = rows
  .map(
    (r, i) =>
...
const H0 = 406;
const SEL0 = 0x19c;
const PIN = 'bd7bad15e53fe296e790c57803a0d44930e95c7f7db99ee866685fbb5d504f12';
...
const log = `# parallel-batch-54 Log · 8-pick fresh-picks scratch test sweep
...
Pass pin from body-extend-059 Relock: \`${PIN}\`.
...
const summaryPath = path.join(ROOT, 'scripts/_probe/parallel-batch-54-summary.json');
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    {
