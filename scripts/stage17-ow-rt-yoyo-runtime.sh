...
#
# Runs pe_dll_link unit tests + export-compile (bake YOYO compile into sidecar export).
...
cd "$ROOT"
echo "=== Post-v1.0: OW-RT Gate G slice (Linux/cloud: tests + export-compile) ==="
...
WORK="$ROOT/scripts/_stage17-ow-rt-export-compile"
rm -rf "$WORK"
mkdir -p "$WORK"
cp "$TY_FX" "$WORK/input.ty"
set +e
cargo run -q -p verifier --bin emit-rt-sidecar --no-default-features --features full-backends -- --export-compile "$WORK"
EC_EXIT=$?
set -e
if [[ "$EC_EXIT" -ne 0 ]]; then
  echo "emit-rt-sidecar --export-compile expected exit=0 got=$EC_EXIT"
  exit 1
fi
...
assert b"yoyo_export_compile" in b, "missing export-compile marker"
assert b"yoyo_rt.dll" in b, "missing dll name"
o = out.read_bytes()
assert o[:2] == b"MZ" and len(o) >= 64, "output.exe not PE"
print(f"OW_RT_SPIKE yoyo_export_compile=PRESENT path={work} sidecar_bytes={len(b)} output_bytes={len(o)}")
print("OW_RT_SPIKE yoyo_alt_sidecar=EMITTED (export-compile pe_dll)")
print("OW_RT_SPIKE gate_g_slice=export_compile")
PY
echo "OW_RT_SPIKE production_default=RUST"
echo "OW_RT_SPIKE yoyo_export_compile_smoke=SKIP (non-Windows; see Win ps1 for H_00)"
echo "OW_RT_SPIKE yoyo_built=EXPORT_COMPILE yoyo_export_compile=PRESENT disposition=CUT"
echo "OW_RT_SPIKE note=Gate_G_slice_export_compile; call-time not general re-compile; CLOSED requires production YOYO-built compile sidecar + no Rust yoyo_rt.dll host trust"
