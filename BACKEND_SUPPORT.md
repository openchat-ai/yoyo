...
|------|--------|
| **Hole** | **OW-STUB** |
| **Change** | H_00 stub **82→69** B span: drop export-dir guards + AddressOfNameOrdinals walk; resolve **AddressOfFunctions[0]** directly (`yoyo-runtime` `.def` export order pin) |
...
| **Floor** | **69B** nz span (71B raw) = LoadLibraryA + AddressOfFunctions[0] + ExitProcess×2; below needs OW-IAT/OW-RT not stub tweak |
| **Gates** | body-ddc + stage14/15/16 stub pin [40,512]; selfhost-body EQUAL unchanged |

### Post-v1.0 path 2 — OW-STUB ordinal-0 export resolve (2026-08-29 · PR #5)
...
| **Obs** | seed PE **248320**; `SEED_HOST sha256_prefix` length **8→16**; inventory evidence `emitter_sha256_prefix=…;seed_sha256_prefix=…;path=h00` |
...
|------|--------|
| **Hole** | **OW-RT** (YOYO-built runtime long pole) |
| **Change** | `ow_rt_yoyo_origin_exit2.ty` RAW_BYTES+RET → `B8 02 00 00 00 C3`; `pe_dll_link` export body is YOYO-emitted |
...
| **Doc** | `SCOPE-CUT-v1.0-ow-rt-yoyo-runtime.md` |
| **Next** | Sidecar-path RCW slice landed below |

### Post-v1.0 path 2 — OW-RT YOYO sidecar-path R→C→W (Gate G slice · 整仓竣工 · 2026-09-04)

| Item | Detail |
|------|--------|
| **Hole** | **OW-RT** (YOYO-built runtime long pole) |
| **Change** | `yoyo_sidecar_path_rcw` + `emit-rt-sidecar --rcw` places YOYO `yoyo_rt.dll` then host-orchestrated R→C→W under same cwd; `place_cwd_runtime_sidecar` / `write_cwd_runtime_sidecar` env opt-in (default Rust) |
| **Still CUT** | Production default still Rust — **`yoyo_built=SIDECAR_RCW`** · **`yoyo_sidecar_rcw=PRESENT`** · export body still exit-2 · ≠ CLOSED · Gate G checkbox stays open |
| **Gate** | `scripts/stage17-ow-rt-yoyo-runtime.ps1` · Linux `scripts/stage17-ow-rt-yoyo-runtime.sh` · `cargo test -p verifier pe_dll_link` |
| **Doc** | `SCOPE-CUT-v1.0-ow-rt-yoyo-runtime.md` |
| **Next** | Export-compile slice landed below |

### Post-v1.0 path 2 — OW-RT YOYO export-compile (Gate G slice · 整仓竣工 · 2026-09-04)

| Item | Detail |
|------|--------|
| **Hole** | **OW-RT** (YOYO-built runtime long pole) |
| **Change** | `yoyo_sidecar_export_compile` + `emit-rt-sidecar --export-compile` — emit-time YOYO `bootstrap_compile` baked into pe_dll; export (KERNEL32 IAT) writes `output.exe` on call (exits 0/1/2/3) |
