...
**Post-v1.0 path 2（关洞）· OW-RT Linux sidecar parity（2026-08-29）：** Linux H_00 **不再 exact-embed** `libyoyo_runtime.so`；仅嵌 trampoline，cwd sidecar `./libyoyo_runtime.so` + `dlopen`。seed ELF 期望 **≪300000**。**仍 CUT**（Rust `.so` + glibc/libdl tramp）— **禁止**标 CLOSED。
...
| **OW-RT** | Sidecar Rust runtime (Win DLL / Linux `.so`) | **CUT** | 无 exact embed **且** 无 Rust LoadLibrary/libdl sidecar 宿主信任 | Win DLL **≤150000** 观测 **141312**；sidecar `yoyo_rt.dll` / `./libyoyo_runtime.so`；**no exact embed**（双平台） |
