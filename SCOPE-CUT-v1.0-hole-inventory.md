...
**Next tip（post-v1.0 path 2 · 整仓竣工）：** Gate **G 切片** landed dll-recompile（yoyo_built=DLL_RECOMPILE · yoyo_dll_recompile=PRESENT · production_default=RUST · pe_dll 内无 compiler ISA · 仍 CUT · **G 未勾**）。**Next** = embed compile ISA in pe_dll / YOYO-built compile sidecar → flip production default off Rust → inventory CLOSED；**OW-IAT CLOSED** only when yoyo_rt.dll marker absent。
