...
## 观测基线（2026-08-29 · post-v1.0 OW-STUB ordinal-0 + OW-RT sidecar + OW-IAT no-GPA + OW-SEED pin · gate GREEN）
...
| OW-IAT shrink | Win GetProcAddress **ABSENT**（ordinal-0 PE export walk）；Linux tramp dlsym **ABSENT**（ELF dyn walk）；LoadLibraryA/dlopen **PRESENT**（仍 CUT） |
| OW-STUB shrink | ordinal-0 export resolve；was 235B full name walk（仍 CUT） |
| OW-SEED observe | emitter=`yoyo.exe` size+sha256_prefix；seed sha256_prefix≡`SEED_HOST`（16 hex）；path=h00（仍 CUT） |

---

*Stage 16-A · 打破后门魔咒：洞从「v0.9 枚举」变「1.0 FINAL SCOPE-CUT + 可脚本钉」· post-v1.0 OW-RT：Win/Linux exact embed → sidecar（仍 CUT）· post-v1.0 OW-IAT：GetProcAddress → ordinal-0 PE export walk；Linux dlsym → ELF dyn walk（仍 CUT）· post-v1.0 OW-STUB：235→96B ordinal-0（仍 CUT）· post-v1.0 OW-SEED：emitter+seed hash pin（仍 CUT）*
