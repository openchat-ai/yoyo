...
**Next tip（post-v1.0 path 2 · 整仓竣工）：** Gate **G 切片** landed sidecar-path R→C→W（yoyo_built=SIDECAR_RCW · yoyo_sidecar_rcw=PRESENT · production_default=RUST · export 仍 stub · 仍 CUT · **G 未勾**）。**G CLOSED** = production drop Rust sidecar only with evidence；**OW-IAT CLOSED** only when yoyo_rt.dll marker absent。
