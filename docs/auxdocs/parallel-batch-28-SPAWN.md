# parallel-batch-28 SPAWN · scratch-only (post body-extend-033)

> Continuous queue handoff from body-extend-033.
> Pin after Relock: `0f0fce9a754e262914c8e2a78ca2558bd8af31ab0d532339f49018c2354cdac2` (abbrev `0f0fce9a…`).
> Handlers = 204 (H_00..H_197). Last selectors: 0xC4..0xCB = H_190..H_197.
> Source protocol: mirror `docs/auxdocs/parallel-batch-27-log.md` / batch-26 shape.
> No Task tool available on consolidator — this SPAWN is the handoff artifact.
> NOTE: body-extend-033 DDC PE `.text` measured EQUAL this beat — still EXPERIMENTAL · NON-GREEN; do not invent-green.

## Task: parallel-batch-28 (scratch-only)

1. Write `docs/auxdocs/parallel-batch-28-log.md`.
2. Pick 6–8 NEW scratch variants NOT already handlers in `yoyo/projects/yoyo.ty` (after H_190..H_197).
...
4. Prefer fresh slot/imm/alt-slot of INC/DEC/ADD-IMM/SUB-IMM/GET/SET/ADDV/ORV/SUBV/IMUL/CMP/LDB not duplicating H_48–H_197.
5. Scratch-only: do NOT touch yoyo.ty / lock / golden.js / self_test / main.
6. Each pick: write `_scratch_*.ty` + `_scratch_*.code.hex`; verify JS↔Rust byte-eq; record PASS/REJECT in log.
7. After scratches done: append "parent next = body-extend-034 serialize PASSes + 1 Relock" and **immediately spawn** body-extend-034 consolidation Task (same chain protocol), passing pin `0f0fce9a754e2629…` and the PASS list. If no Task tool: write `docs/auxdocs/body-extend-034-SPAWN.md`.
...
- ADD-IMM 52 50 · SUB-IMM 50 48 / 52 48 (complete imm=50 ADD triad; fresh SUB imm=48 at 50/52)
- LDB dd=50 ss=60 oo=70 (H_47 early may exist — verify; if early-locked skip; else oo=78 next rung)
- SET slot/imm not in locked SET set (fresh imm ≠ C0DEC0DE/DEADC0DE/CAFEF00D/…)
- SUB-IMM / ADD-IMM fresh (slot,imm) e.g. imm=58/60
