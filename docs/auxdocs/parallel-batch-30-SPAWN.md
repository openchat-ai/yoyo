# parallel-batch-30 SPAWN · scratch-only (post body-extend-035)

> Continuous queue handoff from body-extend-035.
> Pin after Relock: `23f42236c6097a13e83a15c861d51845fbe1da64eadfabdb95fdeaca3ebe55f5` (abbrev `23f42236…`).
> Handlers = 220 (H_00..H_213). Last selectors: 0xD4..0xDB = H_206..H_213.
> Source protocol: mirror `docs/auxdocs/parallel-batch-29-log.md` / batch-28 shape.
> No Task tool available on consolidator — this SPAWN is the handoff artifact.
> NOTE: body-extend-035 DDC PE `.text` measured EQUAL this beat — still EXPERIMENTAL · NON-GREEN; do not invent-green.

## Task: parallel-batch-30 (scratch-only)

1. Write `docs/auxdocs/parallel-batch-30-log.md`.
2. Pick 6–8 NEW scratch variants NOT already handlers in `yoyo/projects/yoyo.ty` (after H_206..H_213).
...
4. Prefer fresh slot/imm/alt-slot of INC/DEC/ADD-IMM/SUB-IMM/GET/SET/ADDV/ORV/SUBV/IMUL/CMP/LDB not duplicating H_48–H_213.
5. Scratch-only: do NOT touch yoyo.ty / lock / golden.js / self_test / main.
6. Each pick: write `_scratch_*.ty` + `_scratch_*.code.hex`; verify JS↔Rust byte-eq; record PASS/REJECT in log.
7. After scratches done: append "parent next = body-extend-036 serialize PASSes + 1 Relock" and **immediately spawn** body-extend-036 consolidation Task (same chain protocol), passing pin `23f42236c6097a13…` and the PASS list. If no Task tool: write `docs/auxdocs/body-extend-036-SPAWN.md`.
...
- ADD-IMM 51 60 / 52 60 (complete imm=60 ADD triad with H_212)
- SUB-IMM 50 58 / 51 58 / 52 58 (fresh SUB imm=58)
- LDB dd=51/52 ss=60 oo=80 (complete oo=80 LDB triad with H_213; expect imm32 26B)
- SUB-IMM / ADD-IMM fresh imm=68
- LDB oo=88 next rung (dd=50/51/52)
