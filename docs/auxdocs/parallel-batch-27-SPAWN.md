# parallel-batch-27 SPAWN · scratch-only (post body-extend-032)

> Continuous queue handoff from body-extend-032.
> Pin after Relock: `a0cb2642b1b3a3e03be8b82602ae26da1234e8f88170f4c49d836a84caed429d` (abbrev `a0cb2642…`).
> Handlers = 196 (H_00..H_189). Last selectors: 0xBC..0xC3 = H_182..H_189.
> Source protocol: mirror `docs/auxdocs/parallel-batch-26-log.md` / batch-25 shape.
> No Task tool available on consolidator — this SPAWN is the handoff artifact.
> NOTE: body-extend-032 DDC PE `.text` measured EQUAL this beat (prior 031 DIFFER) — still EXPERIMENTAL · NON-GREEN; do not invent-green.

## Task: parallel-batch-27 (scratch-only)

1. Write `docs/auxdocs/parallel-batch-27-log.md`.
2. Pick 6–8 NEW scratch variants NOT already handlers in `yoyo/projects/yoyo.ty` (after H_182..H_189).
...
4. Prefer fresh slot/imm/alt-slot of INC/DEC/ADD-IMM/SUB-IMM/GET/SET/ADDV/ORV/SUBV/IMUL/CMP/LDB not duplicating H_48–H_189.
5. Scratch-only: do NOT touch yoyo.ty / lock / golden.js / self_test / main.
6. Each pick: write `_scratch_*.ty` + `_scratch_*.code.hex`; verify JS↔Rust byte-eq; record PASS/REJECT in log.
7. After scratches done: append "parent next = body-extend-033 serialize PASSes + 1 Relock" and **immediately spawn** body-extend-033 consolidation Task (same chain protocol), passing pin `a0cb2642b1b3a3e0…` and the PASS list. If no Task tool: write `docs/auxdocs/body-extend-033-SPAWN.md`.
...
- ADD-IMM 52 48 · SUB-IMM 52 40 (complete the 50/51/52 triad for imm=48 ADD / imm=40 SUB)
- LDB dd=50/51/52 ss=60 oo=70 (next offset rung; H_47 early may exist — verify)
- SET slot/imm not in locked SET set (fresh imm ≠ DEADC0DE/CAFEF00D/FEEDC0DE/…)
- SUB-IMM / ADD-IMM fresh (slot,imm) e.g. imm=50/52/58
