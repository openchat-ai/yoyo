...
> **Status:** SPIKE Gate G **slice** (export-compile — **not** CLOSED)  
> **Gate:** `scripts/stage17-ow-rt-yoyo-runtime.ps1` · Linux/cloud: `scripts/stage17-ow-rt-yoyo-runtime.sh`  
> **Code:** `yoyo-rust/verifier/src/pe_dll_link.rs` · bin `emit-rt-sidecar` (`--export-compile`) · stub `yoyo/tests/golden/ow_rt_yoyo_origin_exit2.ty` · fixture `yoyo/tests/golden/selfhost_min_nop.ty`  
...
|------|-------------|----------------|
| **OW-RT** | No exact embed **and** no Rust LoadLibrary/libdl / `yoyo_rt.dll` sidecar host trust (YOYO-built runtime) | Sidecar-only shrink; thinner LTO DLL; PE DLL emit spike; YOYO-origin export probe; R→C→W effect harness; alt-sidecar emit; sidecar-path RCW; **export-compile (baked PE, not general re-compile)** |
...
| **G slice** (this tick) | `yoyo_sidecar_export_compile` — emit-time `bootstrap_compile` baked into pe_dll; export (KERNEL32 IAT) writes `output.exe` on call | **CUT** · `yoyo_built=EXPORT_COMPILE` · `yoyo_export_compile=PRESENT` · `gate_g_slice=export_compile` · call-time **not** general re-compile · production_default=RUST |
...
`yoyo_sidecar_export_compile(work_dir)` emit-time compiles via YOYO seed/link, bakes the PE into the sidecar export artifact, then **calls** the export (Win manual-map + KERNEL32 IAT) to write `output.exe`. Honest: call-time does **not** re-compile arbitrary input.  
...
Linux/cloud (unit + export-compile; no Win H_00 smoke):

```bash
bash scripts/stage17-ow-rt-yoyo-runtime.sh
```

Gate prints `OW_RT_SPIKE status=GREEN`, **`yoyo_export_compile=PRESENT`**, **`gate_g_slice=export_compile`**, and **`yoyo_built=EXPORT_COMPILE disposition=CUT`**.  
Win H_00+export-compile no-input may print `yoyo_export_compile_smoke=NOT_STABLE` (AV) — honest; does not fail the gate (export-compile path is the slice evidence).
...
*Post-v1.0 path 2 · 整仓竣工 · OW-RT long pole · Gate G slice = export-compile (not CLOSED)*
