...
| Area | v0.4 includes |
|------|----------------|
| **Runtime.dll surface** | DLL **485888?231936** B fail-closed (`stage10-runtime-surface.ps1`); gen12 SHA `43ffde58…` |
| **Linux pure M4** | gen1?gen4 without `bootstrap --selfhost`; gen4?gen3_direct EQUAL (`stage10-linux-pure-m4.sh`) |
| **asm peer I/O parity** | win32 `0x20/0x50/0x51` byte-equal Rust/JS; `stage10-asm-peer-io.ps1` |
| **v0.3 baseline** | H_00 · JS peer · Win pure M4 · fullbody · lock · gen12 — still green |
...
| **gen12 `.text` (Win)** | **`43ffde58…`** · **18432** bytes |
| **runtime.dll** | **231936** B (still Rust-built; outside gen12 window) |
| **Linux M4 ELF** | `085d07d4…` · 704512 B (pure path, no `--selfhost`) |
...
**??????** — more bytes under DDC+Lock, fewer "green only because host wrapped it" paths. Detection bar, not proof.
