...
**Post-v1.0 OW-H00 re-eval（PR #8）：** JS template at canonical RVAs；**three_peer_full=DIFF**（JS `a9b4cdc8` ≠ Rust `72c27c9f` · 18944 B）。body window **EQUAL**。**OW-H00 CUT** — CLOSED 仅当 three_peer_full EQUAL。
...
| **OW-STUB** | H_00 manual-map stub tail | `.text` emit 后 | `stub_tail_nonzero` 钉 **[40, 950]**；观测 **905** B（CreateFile prelude + PE map） | **CUT** — 窗外；JS/Rust nz 同但 full `.text` DIFF |
...
## 观测基线（2026-08-29 · post PR #8 · OW-IAT manual-map wire-up）

| Monitor | Value |
|---------|-------|
| selfhost-body compared | **17805** B EQUAL |
| full `.text` JS↔Rust | **DIFF**（`a9b4cdc8` vs `72c27c9f`） |
